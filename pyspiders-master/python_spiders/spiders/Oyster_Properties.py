...
class spiderQoutes(scrapy.Spider):
    ...
    def get_property_details(self,response,**kwargs):
        ...
        description = soup.find('p',class_='room').text.replace('*ZERO DEPOSIT AVAILABLE**\n\n','').replace('*','')
        ...
        from datetime import datetime
        import dateparser
        available_date = "".join(response.xpath("//p[contains(@class,'room')]//text()[contains(.,'Available ')]").getall())
        if available_date:
            if not "now" in available_date.lower():
                available_date = available_date.lower().split("available")[1].replace("end","").replace("of","").replace("week","").replace("!","").replace("onwards","").replace("from","").strip().replace("  "," ")
