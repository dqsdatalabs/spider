...
class KlImmoWebDeSpider(scrapy.Spider):
    name = "kl_immo_web_de"
    start_urls = ['https://portal.immobilienscout24.de/ergebnisliste/26472471']
    #allowed_domains = ["kl-immo-web.de"]
    ...
    def parse(self, response, **kwargs):
        for url in list(set(['https://portal.immobilienscout24.de'+i for i in response.css('a::attr(href)').extract() if 'expose' in i])):
    ...
    def populate_item(self, response):
        ...
        description = ' '.join([i.strip() for i in response.css('.expose--text:nth-child(6) p::text').extract() if i.strip()!=''])
        description = description_cleaner(description)
        description = description.replace('kl immobilien gmbh kl immobilien gmbh telefon:','')
        ...
        # info
        labels = [i for i in response.css('li p:nth-child(1)::text').extract() if i not in ['Balkon/ Terrasse:', 'Keller:', 'Einbauküche:']]
        vals = response.css('li p+ p::text').extract()
        property_type = 'apartment'
        floor = square_meters = deposit = heating_cost = utilities = energy_label = None
        room_count = bathroom_count = 1
        rent = 0
        for i in range(len(vals)):
            if 'Energieeffizienzklasse:' in labels[i]:
                energy_label = vals[i]
            if 'zimmer' in labels[i].lower():
            ...
            if 'Heizkosten:' in labels[i]:
                heating_cost = int(float(vals[i].replace('+','').strip().split()[0].replace('.', '').replace(',', '.')))
        ...
        # # LandLord Details
        item_loader.add_value("landlord_name", 'KL Immobilien GmbH') # String
        item_loader.add_value("landlord_phone", '0208 290120') # String
        item_loader.add_value("landlord_email", 'info@kl-immo-web.de') # String
