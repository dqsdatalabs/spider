...
class MySpider(Spider):
    name = 'century21martinotimmobiliertroyes_com'
    execution_type='testing'
    country='france'
    locale='fr'
    external_source = "Century21martinotimmobiliertroyes_PySpider_france_fr"
    ...
    def start_requests(self):
        start_urls = [
            {"url": "https://www.century21-martinot-immobilier-troyes.com/annonces/location-appartement/", "property_type": "apartment"},
	        {"url": "https://www.century21-martinot-immobilier-troyes.com/annonces/location-maison/v-troyes/", "property_type": "house"},
        ]  # LEVEL 1
    ...
    def parse(self, response):
        ...
        pagination = response.xpath("//ul[contains(@class,'tw-flex')]/li[contains(@class,'suivant')]/a/@href").extract_first()
        if pagination:
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Century21martinotimmobiliertroyes_PySpider_"+ self.country + "_" + self.locale)
        ...
        if not roomcheck:
            room1="".join(response.xpath("//h1//span//text()").getall())
            if room1:
                room1=room1.split("pièce")[0]
        ...
        if price:
            item_loader.add_value("rent_string", price.replace("€","").replace(" ","").strip())
        ...
        available_date = response.xpath("//li[contains(.,'Libre le')]/text()").get()
        if available_date:
            available_date=available_date.split(":")[-1].replace("\xa0","").strip()
            date_parsed = dateparser.parse(available_date)
            if date_parsed:                
                item_loader.add_value("available_date", date_parsed.strftime("%Y-%m-%d"))
        
        item_loader.add_value("landlord_name", "CENTURY 21 Martinot Immobilier")
        item_loader.add_value("landlord_phone", "03 25 82 82 82")
        item_loader.add_value("landlord_email", "troyes@martinot-immobilier.fr")
