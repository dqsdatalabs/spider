...
class MySpider(Spider):
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Era_Bassinpyla_PySpider_france")
        ...
        square_meters=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'Surface')]//following-sibling::div/text()").get()
        if square_meters:
            item_loader.add_value("square_meters", square_meters.split('m²')[0])
        
        room_count=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'pièces')]//following-sibling::div/text()").get()
        if room_count:
            item_loader.add_value("room_count", room_count)

        address = response.xpath("//text()[contains(.,'louer sur ')]").get()
        if address:
            item_loader.add_value("address", address.split('sur ')[-1].strip())
            item_loader.add_value("zipcode", address.split('sur ')[-1].strip().split(' ')[-1].strip())
        ...
        bathroom_count = response.xpath("//div[contains(text(),'Salle de bain')]/following-sibling::div/text()").get()
        if bathroom_count:
            item_loader.add_value("bathroom_count", bathroom_count.strip())

        deposit = response.xpath("normalize-space(//span[contains(text(),'Dépôt de garantie')]/text())").get()
        if deposit:
            item_loader.add_value("deposit", "".join(filter(str.isnumeric, deposit.split('€')[0].split(':')[-1].strip())))

        utilities = response.xpath("normalize-space(//span[contains(text(),'dont charges')]/text())").get()
        if utilities:
            item_loader.add_value("utilities", "".join(filter(str.isnumeric, utilities.split('€')[0].split(':')[-1].strip())))
        
        external_id=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'Référence')]//following-sibling::div/text()").get()
        if external_id:
        ...
        balcony=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'balcony')]//following-sibling::div/text()").get()
        if balcony!='0':
        ...
        garage=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'Garage')]//following-sibling::div/text()").get()
        parking=response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'Parking')]//following-sibling::div/text()").get()
        ...
        phone="".join(response.xpath("//div[@class='contact_nego_boutons_return']/a/font//text()").getall())
        if phone:
            item_loader.add_value("landlord_phone", phone.strip().split(" ")[0])
