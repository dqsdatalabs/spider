...
class MySpider(Spider):
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://raywhitebjr.com.au/properties/residential-for-rent?category=APT%7CUNT&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "https://raywhitebjr.com.au/properties/residential-for-rent?category=HSE%7CTCE%7CTHS&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "house"
            },
            {
                "url" : [
                    "https://raywhitebjr.com.au/properties/residential-for-rent?category=STD&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
    ...
    def populate_item(self, response):
        ...
        if response.url.split("/")[-1] == "2474343": return
        item_loader.add_value("property_type", response.meta.get('property_type'))
        item_loader.add_value("external_link", response.url)
        
        from python_spiders.helper import ItemClear
        ItemClear(response=response, item_loader=item_loader, item_name="external_source", input_value="Raywhitebjr_Com_PySpider_australia", input_type="VALUE")
        ...
        rent = response.xpath("//span[contains(@class,'pdp_price')]/text()[not(contains(.,'Price'))]").get()
        if rent:
            if "pw" in rent.lower():
                rent = rent.lower().split("pw")[0].replace("$","").strip()
                rent = int(rent)*4
        ...
        address = " ".join(response.xpath("//h1[contains(@class,'pdp_address')]//text()").getall())
        if address:
        ...
        name = response.xpath("//script/text()[contains(.,'fullName')]").extract_first()
        if name:
            landlord_name = name.split('"agents":[')[1].split('"fullName":"')[1].split('",')[0]
            item_loader.add_value("landlord_name", landlord_name)
            try:
                item_loader.add_value("landlord_phone", "+"+name.split('"agents":[')[1].split('"mobilePhone":"')[1].split('",')[0])
            except:
                item_loader.add_value("landlord_phone", "+"+name.split('"agents":[')[1].split('"workPhone":"')[1].split('",')[0])
                
            item_loader.add_value("landlord_email", name.split('"agents":[')[1].split('"email":"')[1].split('",')[0])
