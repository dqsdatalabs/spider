...
class spiderQoutes(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        if response.xpath("//span[@class='title' and contains(text(),'LOCATION')]/following-sibling::span/text()[contains(.,'Meublé')]").get(): item["furnished"] = True
        ...
        room_count=response.xpath("//span[contains(.,'Chambres')]/following-sibling::div/span/text()").get()
        if room_count:
            item["room_count"]=room_count
        parking=response.xpath("//a[.='Parking']/text()").get()
        if parking:
            item["parking"]=True
        ...
        values = soup.find('ul', class_='rh_property__additional clearfix')
        if values:
            for row in values.find_all('li'):
