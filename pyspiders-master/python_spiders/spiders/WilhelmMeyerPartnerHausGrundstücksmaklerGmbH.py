...
class WilhelmmeyerpartnerhausgrundstücksmaklergmbhSpider(scrapy.Spider):
    name = "WilhelmMeyerPartnerHausGrundstucksmaklerGmbH"
    start_urls = ['https://www.meyerpartner.de/immobilie/miete/']
    allowed_domains = ["meyerpartner.de"]
    ...
    def populate_item(self, response):
        ...
        for row in zip(keys, vals):
            key = row[0].strip()
            val = ' '.join(row[1].css("::text").getall())
            key = key.lower()
            if 'name' in key:
                ...
                rent, currency = extract_rent_currency(val, self.country, WilhelmmeyerpartnerhausgrundstücksmaklergmbhSpider)
                rent = get_price(val)
            elif "warmmiete" in key:
                total_rent, currency = extract_rent_currency(val, self.country, WilhelmmeyerpartnerhausgrundstücksmaklergmbhSpider)
            ...
            elif 'Energie­effizienz­klasse'.lower() in key:
                energy_label = val
        ...
        description = '\n'.join([x.strip() for x in response.css("div.property-description > div.panel-body>p::text").getall() if 'http' not in x.strip() and 'meyer' not in x.strip()]).strip()
