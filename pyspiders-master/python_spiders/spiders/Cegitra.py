...
class spiderQoutes(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        item["address"] = soup.find('h1').text.split(" - ")[-1].strip()
        item["city"] = soup.find('h1').text.split(" - ")[-1].strip()
        ...
        room = response.xpath('//div[contains(.,"Nombre de pièce(s)")]/following-sibling::div/text()').get()
        if room:
            room = re.sub(r'\D', '', room)
            item["room_count"] = int(room)
        else:
            if 'nombredechambre_s' in info_dict:
                item["room_count"] = int(getSqureMtr(info_dict['nombredechambre_s']))
        ...
        if soup.find("p",class_="honoraires") and getPrice(soup.find("p",class_="honoraires").text):
            ...
            utilities = response.xpath("//div[@class='prices']/p[@class='honoraires']/text()[contains(.,'charges')] | //div[@class='prices']/p[@class='charges']/text()[contains(.,'harges')]").extract_first()
            if utilities:
                uti = utilities.split(":")[-1].strip().split(" ")[0] 
                item["utilities"] = uti 

        meters = response.xpath("//ul[@class='caracteristiques']/li[contains(.,'m²')]/text()").extract_first()
        if meters:
            item["square_meters"] = meters.replace("m²","").strip() 
