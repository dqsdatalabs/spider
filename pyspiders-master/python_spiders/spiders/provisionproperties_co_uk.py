...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        city_zipcode = "".join(response.xpath("//div[contains(@id,'breadcrumbs')]//li/text()").getall())
        if city_zipcode:
            city = city_zipcode.split(",")[-2]
            zipcode = city_zipcode.split(",")[-1].replace("\t","")
            item["city"] = city
            item["zipcode"] = zipcode
        
        item["external_link"] = response.url
        item["external_id"] = response.url.split("lettings/")[1].split("/")[0]
        ...
        from datetime import datetime
        import dateparser
        available_date = "".join(response.xpath("//h3[contains(@class,'property-detail-price')]//span[contains(@class,'note-text')]//text()").getall())
        if available_date:
