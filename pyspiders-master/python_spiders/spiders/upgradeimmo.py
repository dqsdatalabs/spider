...
class UpgradeimmoSpider(scrapy.Spider):
    ...
    def get_property_details(self, response):
        ...
        parking = ''.join(response.xpath("//div[div[contains(.,'Garage')]]/div[@class='value']/text()").extract())
        ...
        item_loader.add_value('external_id', external_link.split("id=")[1].split("&")[0])
        item_loader.add_xpath('address', ".//div[@class='top-title']//div[@class='fleft']//text()")
        item_loader.add_xpath('title', "normalize-space(//title/text())")
        item_loader.add_xpath('bathroom_count', "//div[div[.='Nombre de salle(s) de bain']]/div[@class='value']/text() | //div[div[.='Nombre de salle(s) de douche']]/div[@class='value']/text()")
        ...
        room = response.xpath("//div[div[.='Nombre de Chambre(s)']]/div[@class='value']/text()").extract_first()
        if room:
            item_loader.add_xpath('room_count', room.split(" ")[0])
