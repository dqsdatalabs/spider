...
class SwaImmobilienGmbhSpider(scrapy.Spider):
    name = "SWA_Immobilien_GmbH"
    start_urls = ['https://api.immowelt.com/auth/oauth/token']
    allowed_domains = ["api.immowelt.com", 'immowelt.de', 'rtl-immobilien.de']
    country = 'germany'
    locale = 'de'
        ...
        if 'areas' in data:
            if 'livingArea' in data['areas']:
                if 'value' in data['areas']['livingArea']:
                    square_meters = data['areas']["livingArea"]["value"]
        ...
        if 'price' in data:
            if 'value' in data['price']:
                if data['price']['value'] == None:
                    return
                rent = data['price']["value"]
            if 'type' in data['price']:
                if 'rent' not in data['price']["type"]:
                    return
        jsn = response.css("script[type='application/json']::text").get()
        heat_index = jsn.find("Heizkosten")
        if heat_index != -1:
            start = heat_index + len("&q;,&q;StringValue&q;:&q")
            heat = jsn[start:start+40]
            if "in Warmmiete enthalten" in heat:
                heating_cost = True
