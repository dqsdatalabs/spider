...
class MySpider(Spider):
    name = 'mijnhuisenik_com'
    execution_type = 'testing'
    country = 'netherlands'
    locale = 'nl'
    start_urls = ["https://www.mijnhuisenik.com/nl/realtime-listings/consumer"]
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Mijnhuisenik_PySpider_netherlands")
        ...
        description = " ".join(response.xpath("//p[contains(@class,'desc')]//text()").getall()).strip()   
        if description:
        ...
        images = [response.urljoin(x) for x in response.xpath("//div[contains(@class,'main-gallery')]//@data-src").getall()]
        if images:
        ...
        utilities = response.xpath("//p[contains(@class,'desc')]//text()[contains(.,'Servicekosten')]").get()
        if utilities:
            utilities= utilities.split(",")[0].strip().replace("\xa0"," ").split(" ")[-1]
        ...
        item_loader.add_value("landlord_name", "Mijn Huis EN IK")
        item_loader.add_value("landlord_phone", "043 851 25 25")
        item_loader.add_value("landlord_email", "info@mijnhuisenik.com")
