...
class Saarbruecken_homecompanyDeSpider(scrapy.Spider):
    name = 'saarbruecken_homecompany_de'
    allowed_domains = ['saarbruecken.homecompany.de']
    start_urls = ['https://saarbruecken.homecompany.de/en/suchergebnis/noreset/1']  # https not http
    country = 'germany'
    locale = 'de'
    ...
    keywords = {
        ...
        'washing_machine': ['waschen', 'w scherei', 'waschmaschine','waschk che','washing'],
    ...
    def parse(self, response, **kwargs):
        ...
        for rental in rentals:
            yield Request(url='https://saarbruecken.homecompany.de/'+rental,
                          callback=self.populate_item)
        if response.css('.nextpage'):
            next_page = response.css('.nextpage::attr(href)').extract_first()
            yield Request(url='https://saarbruecken.homecompany.de/'+next_page,
                          callback=self.parse)
    ...
    def populate_item(self, response):
        ...
        amenities = (" ".join(response.css("th:contains('Furniture features') + td::text").extract_first())).lower()
        ...
        images = ['https://saarbruecken.homecompany.de/'+i for i in images]
        ...
        utilities = response.css("div.price:contains('dditional cost') span.amount::attr(data-price)").extract_first()

        floor = response.css("li:contains('Floor:')::text").extract_first()

        energy_label = (response.css("li:contains('Energy efficiency class:')::text").extract_first())
        if energy_label:
        ...
        if any(word in remove_unicode_char(description.lower()) for word in self.keywords['parking']):
            ...
            parking = response.css("td:contains('Stellplatz')::text").extract_first()
            if parking:
                parking = True
            else:
                parking = False
        ...
        if any(word in remove_unicode_char(description.lower()) for word in self.keywords['terrace']):
            ...
            terrace = response.css("td:contains('Terrasse')::text").extract_first()
            if terrace:
                terrace = True
            else:
                terrace = False
        ...
        if any(word in remove_unicode_char(description.lower()) for word in self.keywords['washing_machine']) or any(word in remove_unicode_char(amenities.lower()) for word in self.keywords['washing_machine']):
            washing_machine = True

        dishwasher = None
        if any(word in remove_unicode_char(description.lower()) for word in self.keywords['dishwasher']) or any(word in remove_unicode_char(amenities.lower()) for word in self.keywords['dishwasher']):
            dishwasher = True

        landlord_name = "saarbruecken homecompany"
        landlord_phone = "+ 49 (0) 234 - 19445"
        landlord_email = "saarbruecken@homecompany.de"
