...
class KmwimmobilienSpider(scrapy.Spider):
    name = "KMWImmobilien"
    start_urls = ['https://api.immowelt.com/auth/oauth/token']
    allowed_domains = ["api.immowelt.com", "immowelt.de", "kmw-immobilien.de"]
    ...
    def populate_item(self, response):
        ...
        for area in equipments_area:
            for equipement in area['Equipments']: 
                if equipement['Key'] == 'FLOOR':
                    ...
                    if 'rücksprache' in val.lower() or 'vereinbarung' in val.lower():
                        available_date = None
                    elif 'sofort' in val.lower():
                        available_date = datetime.now().strftime("%Y-%m-%d")
                    else:
                        available_date = parse(val).strftime("%Y-%m-%d")
                elif equipement['Key'] == 'PARKINGSLOT':
                    parking = True
        ...
        landlord_email = 'info@kmw-immobilien.de'
        ...
        if jsn[f"expose/{external_id.lower()}"]["EnergyPasses"]:
            for item in jsn[f"expose/{external_id.lower()}"]["EnergyPasses"]:
                if item['Data'] and 'Class' in item['Data'][0]:
        ...
        # # MetaData
        item_loader.add_value("external_link", 'https://kmw-immobilien.de/immobilienangebot.htm#/expose' + data['globalObjectKey']) # String
