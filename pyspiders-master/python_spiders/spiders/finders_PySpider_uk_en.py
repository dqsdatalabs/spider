...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self,response,**kwargs):
        ...
        zipcode = response.xpath("//h1[contains(@class,'fleft')]/text()").get()
        if zipcode:
            item["zipcode"] = zipcode.split("-")[-1].strip()
            item["city"] = zipcode.split("-")[-2].split(",")[-1].strip()
        ...
        if parking and "None" not in parking:
            item["parking"] = True

        deposit = response.xpath("//li[contains(.,'Security deposit')]/span//text()").get()
        if deposit:
            deposit = deposit.split("£")[1].split(".")[0].strip().replace(",","")
            item["deposit"] = deposit
        
        floor_plan_images = [ response.urljoin(x) for x in response.xpath("//a[@class='floorplan']/@href").getall()]
        if floor_plan_images:
            item["floor_plan_images"] = floor_plan_images
        
        status = response.xpath("//div/@class[contains(.,'seal_underoffer')]").get()
        if not status:
            yield item