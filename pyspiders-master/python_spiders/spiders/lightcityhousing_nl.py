...
class MySpider(Spider):
    name = 'lightcityhousing_nl'
    execution_type = 'testing'
    country = 'netherlands'
    locale = 'nl'
    start_urls = ["https://www.lightcityhousing.nl/nl/realtime-listings/consumer"]
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Lightcityhousing_PySpider_netherlands")
        ...
        description = " ".join(response.xpath("//div[@class='description-content']//text()").getall()).strip()   
        if description:
        ...
        images = [response.urljoin(x) for x in response.xpath("//div[@class='swiper-wrapper']//img[2]/@src").getall()]
        if images:
        ...
        item_loader.add_xpath("deposit", "//dt[contains(.,'Borg')]/following-sibling::dd[1]/text()")
        ...
        item_loader.add_value("landlord_name", "Lightcity Housing")
        item_loader.add_value("landlord_phone", "+31(0)40 243 00 30")
        item_loader.add_value("landlord_email", "info@lightcityhousing.nl")
