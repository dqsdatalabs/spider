...
class auditaSpider(scrapy.Spider):
    ...
    def parse1(self, response, **kwargs):
        ...
        for ech_prop in response.xpath("//div[@class='product-listing']/a/@href").getall():
            external_link = "http://www.gifparis.com"+ech_prop.replace("..","")
            yield scrapy.Request(url = external_link, callback = self.get_property_details, meta = {"external_link":external_link})
        next_page = soup.find("div", class_="pagination-listing").find("li", class_="next-link active")
        if next_page:
        # for ech_pg in range(1,page_count):
            ...
            yield scrapy.Request(url = "http://www.gifparis.com"+next_page.find("a")["href"], callback = self.parse1)

    # def parse2(self, response, **kwargs):
    #     soup = BeautifulSoup(response.body)
    #     for ech_prop in soup.find_all("div", class_="item-product-listing"):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        item["external_id"] = response.xpath("substring-after(//span[contains(.,'Ref.')]/text(),':')").get().strip()
        
        desc = soup.find("div",class_="product-description").text.strip()
        if desc:
        ...
        address = soup.find("span", class_="alur_location_ville").text
        if address:
            item["address"] = address

        image_list = []
        for ech_im in soup.find("div", class_="container-slider-product").find_all("div", class_="item-slider"):
        ...
        temp_rent = re.findall(r'\d+',soup.find("span", class_="alur_loyer_price").text)
        rent = ""
        if len(temp_rent)>1:
        ...
        temp_dic = {}
        for ech_det in response.xpath("//li[contains(@class,'list-group-item')]/div"):
            temp_dic[ech_det.xpath("./div[@class='col-sm-6'][1]/text()").get()] = ech_det.xpath("./div[@class='col-sm-6']/b/text()").get()
        ...
        if "chambres" in temp_dic:
            item["room_count"] = int(temp_dic["nombrepi_ces"])
        elif "nombrepi_ces" in temp_dic:
            item["room_count"] = int(temp_dic["nombrepi_ces"])
            
        if "salle_s_debains" in temp_dic:
            item["bathroom_count"] = int(temp_dic["salle_s_debains"])
        elif "salle_s_d_eau" in temp_dic:
            item["bathroom_count"] = int(temp_dic["salle_s_d_eau"])
