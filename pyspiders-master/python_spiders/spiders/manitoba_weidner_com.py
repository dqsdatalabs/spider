...
class WeidnerComSpider(scrapy.Spider):
    name = "manitoba_weidner_com"
    start_urls = ['https://manitoba.weidner.com/searchlisting.aspx']
    allowed_domains = ["manitoba.weidner.com"]
    country = 'canada'
    locale = 'en'
    ...
    def populate_item(self, response):
        ...
        images = [i for i in ['https://cdngeneral.rentcafe.com/' + i.split('.jpg')[0]+'.jpg' for i in response.css('img::attr(src)').extract() if 'https:' not in i and '.png' not in i] if ' ' not in i]
        room_count = 1
        ...
        elevator_names = [
         'elevator']
        if re.search('(?=(' + '|'.join(pets_names) + '))', description.lower()):
            pets_allowed = True
        if re.search('(?=(' + '|'.join(balcony_names) + '))', description.lower()):
            balcony = True
        if re.search('(?=(' + '|'.join(terraces) + '))', description.lower()):
            terrace = True
        if re.search('(?=(' + '|'.join(washing_names) + '))', description.lower()):
            washing_machine = True
        if re.search('(?=(' + '|'.join(dishwasher_names) + '))', description.lower()):
            dishwasher = True
        if re.search('(?=(' + '|'.join(park_names) + '))', description.lower()):
            parking = True
        if re.search('(?=(' + '|'.join(elevator_names) + '))', description.lower()):
            elevator = True
        if re.search('(?=(' + '|'.join(pool_names) + '))', description.lower()):
            swimming_pool = True
