...
class ExpertimmoSpider(scrapy.Spider):
    name = "expertimmo"
    allowed_domains = ["expertimmo.be"]
    start_urls = (
        "https://www.expertimmo.be/nl/te-huur/#Type=1",
        # "https://www.expertimmo.be/nl/te-huur/#Type=3",
    )
    ...
    def parse(self, response, **kwargs):
        ...
        yield JsonRequest(
            url="https://www.expertimmo.be/Modules/ZoekModule/RESTService/SearchService.svc/GetPropertiesJSON/0/0",
            data={
                "bedrooms": "0",
                "CheckedTypes": "0",
                "City": "0",
        ...
        for obj in json_data:
            if obj["Property_URL"] and len(obj["Property_URL"]) > 2:
                ...
                yield scrapy.Request(
                    "https://www.expertimmo.be/nl" + obj["Property_URL"],
                    self.parse_detail,
                    dont_filter=True,
    ...
    def parse_detail(self, response, obj):
        ...
        if len(main_block) == 1:
            ...
            if response.xpath("//h2[contains(.,'Studio')]/text()").get():
            ...
            if len(price) != 2:
                ...
                item_loader.add_value("currency", currency_parser(price[0], "{}_PySpider_{}_{}".format(self.name.capitalize(), self.country, self.locale)))
            ...
            if dt:
                if "," in dt:
                    dt = dt.split(",")[0].strip()
                date_parsed = dateparser.parse(dt.replace("vanaf","").strip(), date_formats=["%m/%d/%Y"])
                if date_parsed:
