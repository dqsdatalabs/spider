...
class MySpider(Spider):
    name = 'era_marseille_10_com'
    execution_type='testing'
    country='france'
    locale='fr'
    # DYNAMIC DATA
    b_url = "http://www.immobilier-marseille-10-era-adequat.com"
    source_name = "Era_Marseille_10_PySpider_france"
    # --------------
    ...
    def populate_item(self, response):
        ...
        if response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'chambre')]//following-sibling::div/text()").get():
             ItemClear(response=response, item_loader=item_loader, item_name="room_count", input_value="//div[@class='criteres-list']/ul/li/div[contains(.,'chambre')]//following-sibling::div/text()", input_type="F_XPATH", get_num=True)
        elif response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'pièces')]//following-sibling::div/text()").get():            
        ...
        if response.xpath("normalize-space(//span[contains(text(),'dont charges')]/text())").get():
        ...
        elif response.xpath("//div[contains(@class,'description principale')]/text()[contains(.,'charges comprises dont')]").get():
            ItemClear(response=response, item_loader=item_loader, item_name="utilities", input_value="//div[contains(@class,'description principale')]/text()[contains(.,'charges comprises dont')]", input_type="F_XPATH", get_num=True, split_list={"dont":1,"€":0})
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_name", input_value="ERA ADEQUAT IMMOBILIER", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_phone", input_value="04 91 31 74 02 ", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_email", input_value="mentions-legales@erafrance.com ", input_type="VALUE")
