...
class Bender_immobilien_deSpider(Spider):
    name = 'bender_immobilien_de'
    country='germany'
    locale='de' 
    external_source = "{}_PySpider_{}_{}".format(name.capitalize(), country, locale)
    execution_type='testing'
    allowed_domains = ["www.bender-immobilien.de"]
    start_urls = ["https://www.bender-immobilien.de/kaufen/alle-angebote/?mt=rent&category=&city=&address=#immobilien"]
    position = 1
    ...
    def populate_item(self, response):
        ...
        parking = information_selector.css("span.key:contains('Stellplätze') + span.value::text").get()
        if(parking):
            parking = True
        else:
            parking = False
        ...
        if (not landlord_name):
            landlord_name = "Bender Immobilien"
        
        landlord_phone = response.css("p:contains('Telefon:')::text").get()
        if(landlord_phone):
            landlord_phone = landlord_phone.split(":")[1]
        else: 
            landlord_phone = "0800 / 11 88 660"
        
        landlord_email = "info@bender-immobilien.de"
