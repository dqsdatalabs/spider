...
class Wohnwelt_immobilienDeSpider(scrapy.Spider):
    name = 'wohnwelt_immobilien_de'
    allowed_domains = ['wohnwelt-immobilien.de']
    start_urls = ['https://www.wohnwelt-immobilien.de/immobilien-mieten.xhtml']  # https not http
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response, **kwargs):
        ...
        for rental in rentals:
            if not rental.css('.status-rented') and not rental.css('.status-reserved'):
                external_link = rental.css('.image a::attr(href)').extract_first()
                yield Request(url='https://www.wohnwelt-immobilien.de/'+external_link,
    ...
    def populate_item(self, response):
        ...
        landlord_email = 'info@wohnwelt-immobilien.de'
        landlord_phone = response.css('.center span span::text').extract_first()
        description = ((((' '.join(response.css('span:nth-child(1) span::text').extract()).replace('\n','')).replace('\t','')).replace('\r','')))
        # # MetaData
