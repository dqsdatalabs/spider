...
class MySpider(Spider):
    name = 'century21esneuilly_com'
    execution_type='testing'
    country='france'
    locale='fr'
    external_source = "Century21esneuilly_PySpider_france_fr"
    custom_settings = {
    ...
    def start_requests(self):
        start_urls = [
            {"url": "https://www.century21-es-neuilly.com/annonces/location/?types_biens=appartement&xhr=true", "property_type": "apartment"},
	        {"url": "https://www.century21-es-neuilly.com/annonces/location/?types_biens=maison&xhr=true", "property_type": "house"},
        ]  # LEVEL 1
    ...
    def populate_item(self, response):
        ...
        studio_type= response.xpath("//span[contains(.,'Type d') and contains(.,'appartement')]/following-sibling::text()").get()
        if studio_type:
            if "studio" in studio_type.lower() or "studette" in studio_type.lower():
                prop_type = "studio"
        ...
        utilities = response.xpath("//p[contains(.,'Charges forfaitaires')]/text()").get()
        if utilities:
        ...
        item_loader.add_value("landlord_name", "CENTURY 21 Eric Sellier")
        item_loader.add_value("landlord_phone", "01 47 22 50 52")
        item_loader.add_value("landlord_email", "agencesellier@century21.fr")
