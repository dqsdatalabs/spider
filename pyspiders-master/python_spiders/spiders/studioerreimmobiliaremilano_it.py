...
class MySpider(Spider):
    ...
    def populate_item(self, response):
        ...
        all_text = str(data["html"])
        lat = re.search("LatLng\(([\d.]+),",all_text).group(1)
        long = re.search(", ([\d.]+)\);",all_text).group(1)
        if lat:
