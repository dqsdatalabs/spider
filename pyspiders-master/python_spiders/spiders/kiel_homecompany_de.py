...
class Kiel_homecompanyDeSpider(scrapy.Spider):
    name = 'kiel_homecompany_de'
    allowed_domains = ['kiel.homecompany.de']
    start_urls = ['https://kiel.homecompany.de/en/suchergebnis/noreset/1']  # https not http
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response, **kwargs):
        ...
        for rental in rentals:
            yield Request(url='https://kiel.homecompany.de/'+rental,
                          callback=self.populate_item)
        if response.css('.nextpage'):
            next_page = response.css('.nextpage::attr(href)').extract_first()
            yield Request(url='https://kiel.homecompany.de/'+next_page,
                          callback=self.parse)
    ...
    def populate_item(self, response):
        ...
        images = ['https://kiel.homecompany.de/'+i for i in images]
        ...
        landlord_name = "kiel homecompany"
        landlord_phone = "+ 49 (0) 234 - 19445"
        landlord_email = "kiel@homecompany.de"
