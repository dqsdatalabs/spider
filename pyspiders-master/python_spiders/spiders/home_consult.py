...
class HomeConsultSpider(scrapy.Spider):
    ...
    def get_details(self, response):
        ...
        item = ListingItem()
        item['external_source'] = "HomeConsult_PySpider_belgium_fr"
        ...
        item['external_id'] = response.url.split("/")[-2]
        utilities = ''.join(response.xpath(".//div[contains(.//dt//text(), 'Costs')]/dd//text()").extract())
        if utilities:
