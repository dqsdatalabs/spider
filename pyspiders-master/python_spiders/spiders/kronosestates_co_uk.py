...
class MySpider(Spider):
    name = 'kronosestates_co_uk'
    execution_type = 'testing'
    country = 'united_kingdom'
    locale = 'en'
    start_urls = ["https://www.kronosestates.co.uk/notices.php?c=44&p=1&available_date=undefined&filter_attribute[numeric][2][min]="]

    # 1. FOLLOWING
    def parse(self, response):
        page = response.meta.get("page", 2)
        seen = False
        for item in response.xpath("//div[@class='property_thumb']"):
            status = item.xpath(".//div[contains(@class,'property_badge')]//text()").get()
            if status and (status == "On Hold" or status == "Let"):
                pass
            else:
        ...
        if page == 2 or seen:
            p_url = f"https://www.kronosestates.co.uk/notices.php?c=44&p={page}&&filter_attribute%5Bnumeric%5D%5B2%5D%5Bmin%5D="
            yield Request(
                p_url,
                callback=self.parse,
                meta={"page":page+1})
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Kronosestates_Co_PySpider_united_kingdom")
        ...
        washing_machine = response.xpath("//li//div[contains(.,'Washing') or contains(.,'Washer')]/text()").get()
        if washing_machine:
            item_loader.add_value("washing_machine", True)
        dishwasher = response.xpath("//li//div[contains(.,'Dishwasher')]/text()").get()
        if dishwasher:
            item_loader.add_value("dishwasher", True)
        elevator = response.xpath("//li//div[contains(.,'Elevator')]/text()").get()
        if elevator:
        ...
        item_loader.add_value("landlord_name", "Kronos Estate Agents")
        item_loader.add_value("landlord_phone", "0208 004 4477")
        item_loader.add_value("landlord_email", "lettings@kronosestates.co.uk")
