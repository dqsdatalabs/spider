...
class Juber_immobilien_deSpider(Spider):
    name = 'juber_immobilien_de'
    country='germany'
    locale='de' 
    external_source = "{}_PySpider_{}_{}".format(name.capitalize(), country, locale)
    execution_type='testing'
    allowed_domains = ["www.juber-immobilien.de"]
    start_urls = ["https://www.juber-immobilien.de/immobilienangebote/?mt=rent&category=&address=#immobilien"]
    position = 1

    def parse(self, response):
        for url in response.css("div.immo-listing__wrapper a.immo-listing__image::attr(href)").getall():
    ...
    def populate_item(self, response):
        ...
        if(
            ...
            or "im zentrum von wissen" in lowered_title
        ):
            return
        ...
        description = information_selector.css("b-tab[title='Beschreibung'] p::text").getall()
        ...
        external_id = information_selector.css("span.key:contains('Objekt-Nr:') + span.value::text").get()
       
        landlord_name = "juber-immobilien"
        landlord_phone = "0214 / 860 900 10"
        landlord_email = "info@juber-immobilien.de"
