...
class laforet(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        rent = response.xpath("//li[contains(.,'loyer mensuel charge')]/span[@class='valeur']/text()").get()
        if rent:
            item["rent"] =rent.replace("\xa0","").split(",")[0]
        room_count = response.xpath("//div[contains(@class,'fichebienpicto2')][contains(.,'chambre')]//text()").get()
        if room_count:
            room_count = room_count.strip().split(" ")[0]
            item["room_count"] = int(room_count)
        else:
            room_count = response.xpath("//div[contains(@class,'fichebienpicto2')][contains(.,'pièce')]//text()").get()
            if room_count:
                room_count = room_count.strip().split(" ")[0]
                item["room_count"] = int(room_count)
