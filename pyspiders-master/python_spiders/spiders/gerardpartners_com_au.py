...
class MySpider(Spider):
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://www.gerardpartners.com.au/renting/properties-for-lease/?property_type%5B%5D=Apartment&property_type%5B%5D=Terrace&property_type%5B%5D=Unit&min_price=&max_price=&bedrooms=&bathrooms=&carspaces=",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "https://www.gerardpartners.com.au/renting/properties-for-lease/?property_type%5B%5D=House&min_price=&max_price=&bedrooms=&bathrooms=&carspaces=",
                ],
                "property_type" : "house"
            },
            {
                "url" : [
                    "https://www.gerardpartners.com.au/renting/properties-for-lease/?property_type%5B%5D=Studio&min_price=&max_price=&bedrooms=&bathrooms=&carspaces=",
                ],
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source","Gerardpartners_Com_PySpider_australia")
        ...
        if rent and "$" in rent:
            price= rent.replace("Per","").replace("Week","").replace("P","").replace("/"," ").replace(",","").split("$")[1].strip().split(" ")[0]
        ...
        parking = response.xpath("//li[contains(.,'Car')]//div//text()").get()
        if parking:
        ...
        landlord_email = response.xpath("//div[contains(@class,'agent-detail')]//p[contains(@class,'email')]").get()
        if landlord_email:
            item_loader.add_value("landlord_email", landlord_email.strip())
        
        landlord_phone = response.xpath("//div[contains(@class,'agent-detail')]//p[contains(@class,'phone')]").get()
        if landlord_phone:
