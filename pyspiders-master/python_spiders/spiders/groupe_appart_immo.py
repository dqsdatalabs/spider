...
def getPrice(text):
    price = text.split("/")[0].split("Loyer")[1].strip().replace("\xa0","")
    return int(float(price.replace("\x80","")))
...
class laforet(scrapy.Spider):
    name = 'groupe_appart_immo_PySpider_france_fr_disabled'
    ...
    def get_property_details(self, response, **kwargs):
        ...
        if "chambres" in temp_dic:
            rooms = getSqureMtr(temp_dic["chambres"])
        ...
        if "salle_s_debains" in temp_dic:
            ...
            bathrooms = getSqureMtr(temp_dic["salle_s_d_eau"])
            item["bathroom_count"]=bathrooms
        ...
        item["landlord_name"] = "GROUPE APPART IMMO"
        item["landlord_email"] = "contact@appartalyon.com"
