...
class MySpider(Spider):
    name = 'bretigny_sur_orge_guy_hoquet_com'
    execution_type='testing'
    country='france'
    locale='fr'
    # DYNAMIC VALUES
    gh_domain = "https://bretigny-sur-orge.guy-hoquet.com"
    source_name = "Bretigny_Sur_Orge_Guy_Hoquet_PySpider_france"
    landlord_n = "GUY HOQUET BRETIGNY SUR ORGE"
    landlord_p = "01.60.84.00.00"
    # --------------
    headers = {
    ...
    def populate_item(self, response):
        ...
        room_count = response.xpath("//div[@id='intro']//i[contains(@class,'ico king_bed')]/following-sibling::div/text()").get()
        if room_count:
            room_count = room_count.split(" ")[0].strip()
            item_loader.add_value("room_count", room_count)
        else:
            room_count = response.xpath("//div[@id='intro']//i[contains(@class,'ico room')]/following-sibling::div/text()").get()
            if room_count:
                room_count = room_count.split(" ")[0].strip()
