...
class MySpider(Spider):
    name = 'first4homesni_co_uk'
    execution_type='testing'
    country='united_kingdom'
    locale='en'
    external_source='First4homesni_Co_PySpider_united_kingdom'
    def start_requests(self):

        start_urls = [
            {
                "url" : [
                    "http://www.first4homesni.co.uk/property-for-rent?q=&st=rent&lp=0&up=0&beds=&radius=0&town=&sta=5&sta=6&sta=7&sty=1",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "http://www.first4homesni.co.uk/property-for-rent?q=&st=rent&lp=0&up=0&beds=&radius=0&town=&sta=5&sty=8&sty=7&sty=6&sty=5&sty=4&sty=3&sty=2&sty=10",
                ],
                "property_type" : "house"
            },
        ]
    ...
    def parse(self, response):
        ...
        for item in response.xpath("//ul[@class='tLM_body']/li/a/@href").getall():
    ...
    def populate_item(self, response):
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="rent", input_value="//span[@class='xPP_price']/text()", input_type="F_XPATH", get_num=True)
        ...
        desc = " ".join(response.xpath("//div[contains(@class,'description')]//*[self::p or self::ul]//text()").getall())
        if desc:
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="images", input_value="//div[contains(@class,'xPP_widePhotos ')]//@src", input_type="M_XPATH")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_name", input_value="First 4 Homes", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_phone", input_value="028 2766 3456", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_email", input_value="hayley@first4homesni.co.uk", input_type="VALUE")
