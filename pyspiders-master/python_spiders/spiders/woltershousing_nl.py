...
class MySpider(Spider):
    name = 'woltershousing_nl'
    execution_type = 'testing'
    country = 'netherlands'
    locale = 'nl'
    start_urls = ["https://www.woltershousing.nl/nl/realtime-listings/consumer"]
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Woltershousing_PySpider_netherlands")
        ...
        if response.meta.get("room_count"): item_loader.add_value("room_count", response.meta.get("room_count"))
            ...
            room_count = response.xpath("//dt[contains(.,'Aantal kamers')]/following-sibling::dd[1]/text()").get()
            if room_count:
        ...
        available_date = response.xpath("//dt[contains(.,'Status')]/following-sibling::dd[1]/text()[.!='Onder optie']").get()
        if available_date:
            ...
            available_date = response.xpath("//text()[contains(.,'Beschikbaar:')]").get()
            if available_date:
        ...
        deposit = response.xpath("//text()[contains(.,'Waarborgsom:') or contains(.,'Waarborgsom ') and  contains(.,'€')]").get()
        if deposit:
        ...
        item_loader.add_value("landlord_name", "Wolters Housing")
        item_loader.add_value("landlord_phone", "070 361 50 80")
        item_loader.add_value("landlord_email", "info@woltershousing.nl")
