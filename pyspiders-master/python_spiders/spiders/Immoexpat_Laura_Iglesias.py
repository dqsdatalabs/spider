...
class auditaSpider(scrapy.Spider):
    ...
    country = 'fr'
    locale ='en' 
    external_source="Immoexpat_PySpider_france"
    ...
    def get_property_details(self, response):
        ...
        item["city"]=title.split("-")[-2].strip().split(" ")[-1]
        ...
        for ech_h2 in soup.find("div", class_="col-md-9").find_all("h2"):
            ...
            if "communication" in ech_h2.text.lower():
                for ech_tr in ech_h2.find_next_sibling().find_all("tr"):
                    temp_dic[ech_tr.find("th").text] = ech_tr.find("td").text
            if "financial" in ech_h2.text.lower():
                for ech_tr in ech_h2.find_next_sibling().find_all("tr"):
                    temp_dic[ech_tr.find("th").text] = ech_tr.find("td").text
        ...
        if "charges____amount" in temp_dic:
            if temp_dic["charges____amount"]:
                item["utilities"] = str(temp_dic["charges____amount"])
        if "rentalguarantee_numberofmonths" in temp_dic:
            deposit=int(temp_dic["rentalguarantee_numberofmonths"])
            rent=int(getPrice(title.split("-")[-1]))
            item["deposit"]=deposit*rent
