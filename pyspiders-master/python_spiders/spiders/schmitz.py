...
class Schmitz_PySpider_germany_de(scrapy.Spider):
    name = "schmitz"
    start_urls = [
        "https://www.schmitz-immobilienservice.de/immobilien/wohnen/mieten/wohnungen",
        "https://www.schmitz-immobilienservice.de/immobilien/wohnen/mieten/haeuser",
    ]
    allowed_domains = ["schmitz-immobilienservice.de"]
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response):
        ...
        next_page = response.css(".pagination .active+li>a::attr('href')").get()
        if next_page:
    ...
    def populate_item(self, response):
        ...
        sub_title = response.css(".expose > h1+.row p::text").get("").strip()
        external_id = sub_title.replace("Objekt-Nr.", "").split("\n")[0][:-1].strip()
        property_type = None
        if "Wohnung" in sub_title:
            property_type = "apartment"
        elif "Haus" in sub_title:
            property_type = "house"
