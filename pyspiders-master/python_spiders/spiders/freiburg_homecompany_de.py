...
class Freiburg_homecompany_deSpider(Spider):
    name = 'freiburg_homecompany_de'
    country='germany'
    locale='de' 
    external_source = "{}_PySpider_{}_{}".format(name.capitalize(), country, locale)
    execution_type='testing'
    allowed_domains = ["www.freiburg.homecompany.de"]
    start_urls = ["https://freiburg.homecompany.de/en/suchergebnis/noreset/1"]
    position = 1
    ...
    def populate_item(self, response):
        ...
        energy_label = response.css("li:contains('Energy efficiency class')::text").get()
        if(energy_label):
            energy_label = re.findall(r"Energy efficiency class:\s+([A-Z])", energy_label)
        ...
        washing_machine = "washing machine" in features
        dishwasher = "dishwasher" in features
        
        bathroom_count = "bathroom" in features
        if(bathroom_count):
            bathroom_count = "1"
        else:
            bathroom_count = None
        ...
        utilities = response.css("div.price:contains('Additional costs') span.amount::attr(data-price)").get()

        landlord_name = "homecompany"
        landlord_phone = " + 49 (0) 761 - 733-33-37"
        landlord_email = "freiburg@homecompany.de"
