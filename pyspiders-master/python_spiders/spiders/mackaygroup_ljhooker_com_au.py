...
class MySpider(Spider):
    name = 'mackaygroup_ljhooker_com_au'
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://mackaygroup.ljhooker.com.au/search/unit_apartment-for-rent/page-1?surrounding=true",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "https://mackaygroup.ljhooker.com.au/search/house+townhouse+duplex_semi_detached+penthouse+terrace-for-rent/page-1?surrounding=True&liveability=False",
                ],
                "property_type" : "house"
            },
            {
                "url" : [
                    "https://mackaygroup.ljhooker.com.au/search/studio-for-rent/page-1?surrounding=True&liveability=False",
                ],
    ...
    def populate_item(self, response):
        ...
        if response.url == "https://mackaygroup.ljhooker.com.au/404":
            return
        item_loader.add_value("external_source", "Mackaygroup_Ljhooker_Com_PySpider_australia")
        ...
        landlord_name = " ".join(response.xpath("//div[@class='agent-name']//h3//text()").extract())
        if landlord_name:
            item_loader.add_value("landlord_name", re.sub("\s{2,}", " ", landlord_name))

            landlord_phone = " ".join(response.xpath("//span[@class='agent-mobile']//strong/text()").extract())
            if landlord_phone:
                item_loader.add_value("landlord_phone", re.sub("\s{2,}", " ", landlord_phone.replace("\u0081","")))

            landlord_email = " ".join(response.xpath("//script[contains(.,'email') and contains(.,'Person')]").extract())
            if landlord_email:
                if '"@type":"Person"' in landlord_email:
                    item_loader.add_value("landlord_email", landlord_email.split('"@type":"Person"')[-1].split('"email":"')[-1].split('"')[0].strip())
        else:
