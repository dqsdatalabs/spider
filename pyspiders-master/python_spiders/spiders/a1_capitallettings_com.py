...
class MySpider(Spider):
    name = 'a1_capitallettings_com'
    execution_type = 'testing'
    country = 'united_kingdom'
    locale = 'en'
    start_urls = ["https://www.a1-capitallettings.com/property-to-let?start=0"]
    external_source='A1_Capitallettings_PySpider_united_kingdom'
    thousand_separator = ','
    scale_separator = '.'    
    # 1. FOLLOWING
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "A1_Capitallettings_PySpider_united_kingdom")
        ...
        if room_count:
            if room_count != "0":
                ...
                room_count = response.xpath("//img[contains(@src,'receptions')]/following-sibling::strong[1]/text()[.!='0']").get()
                if room_count:
                    item_loader.add_value("room_count", room_count.strip())
        ...
        landlord_name = response.xpath("//div[@id='DetailsBox']//a/b/text()").get()
        if landlord_name:
            item_loader.add_value("landlord_name",landlord_name.strip())
        item_loader.add_value("landlord_email", "info@a1-capitallettings.com")
        item_loader.add_value("landlord_phone", "02071952250")
