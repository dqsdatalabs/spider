...
class ImmobalcaenSpider(scrapy.Spider):
    ...
    def get_details(self, response):
        ...
        if utilities:
            item['utilities'] = int(float(utilities))
        if bathroom_count:
        ...
        if remove_white_spaces(address):
            item['address'] = remove_white_spaces(address)
        else:
            item['address'] = response.xpath("//span[@class='estate-detail-intro__block-text'][2]/text()").get()
        ...
        if rent:
            item['rent'] = rent.replace("€","").replace(" ","")
            item['currency'] = "EUR"
