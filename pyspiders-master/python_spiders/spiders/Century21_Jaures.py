...
class spiderQoutes(scrapy.Spider):
    ...
    country = 'france'
    locale ='fr'
    external_source = "Century21_Jaures_Boulogne_PySpider_france"

    def start_requests(self):
        start_url = [
            {
                'url': 'https://www.century21-jaures-boulogne.com/annonces/location/#/annonces/location-appartement/',
                'property_type': 'apartment'
            },
            {
                'url': 'https://www.century21-jaures-boulogne.com/annonces/location/#/annonces/location-maison/',
                'property_type': 'house'
            },
            ]
    ...
    def parse(self, response, **kwargs):
        if response.xpath("//div[contains(@class,'c-the-property-thumbnail')]/a/@href").extract_first():
            for item in response.xpath("//div[@class='tw-w-full md:tw-w-auto tw-flex-grow']/a[contains(@class,'c-the-button')]/@href").getall():
                follow_url =  "https://www.century21-jaures-boulogne.com/" + item + "?came_from=/annonces/location/"
                yield Request(follow_url, callback=self.populate_item, meta={'property_type': response.meta.get('property_type')})
        else:
            for item in response.xpath("//div[contains(@class,'c-the-property-thumbnail')]//div"):
                follow_url =  "https://www.century21-jaures-boulogne.com/" + item.xpath(".//@href").get() + "?came_from=/annonces/location/"
                yield Request(follow_url, callback=self.populate_item, meta={'property_type': response.meta.get('property_type')})
    ...
    def populate_item(self, response):
        ...
        status = response.xpath('//span[@class="tw-block c-text-theme-heading-1"][1]/text()').get()
        if status and 'parking' in status.strip().lower():
            return
        ...
        item_loader.add_value("landlord_name", "CENTURY 21 Agence Jaurès")
        item_loader.add_value("landlord_phone", "01 46 10 42 20")
