...
class MySpider(Spider):
    name = 'erastimartinique_com'
    execution_type='testing'
    country='france'
    locale='fr'
    external_source='Erastimartinique_PySpider_france_fr'
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Erastimartinique_PySpider_"+ self.country + "_" + self.locale)

        rent="".join(response.xpath("//div[@class='prix loyer']/span[1]/text()").getall())
        if rent:
            price=rent.replace("\xa0","").split("€")[0].strip().split(" ")[-1]
            print(price)
        ...
        bathroom_count = response.xpath("//div[@class='criteres-list']/ul/li/div[contains(.,'Salle')]//following-sibling::div/text()").get()
        if bathroom_count:
