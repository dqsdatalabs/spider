...
class BestedmontonapartmentSpider(scrapy.Spider):

    name = "bestedmontonapartments"
    ...
    def start_requests(self):
        url = f'https://api.theliftsystem.com/v2/search?locale=en&client_id=485&auth_token=sswpREkUtyeYjeoahA2i&city_id=845&geocode=&min_bed=-1&max_bed=100&min_bath=0&max_bath=10&min_rate=0&max_rate=1300&min_sqft=0&max_sqft=10000&show_custom_fields=true&show_promotions=true&region=&keyword=false&property_types=apartments,+houses,+commercial&ownership_types=&exclude_ownership_types=&custom_field_key=&custom_field_values=&order=min_rate+ASC&limit=66&neighbourhood=&amenities=&promotions=&city_ids=&pet_friendly=&offset=0&count=false'
    ...
    def parse(self, response):
        ...
        for apartment in apartments:
            ...
            i=1
            for suite in request.css(".suite-wrap"):
                if 'not' in suite.css(".suite-availability span::text").get().lower():continue

                regx = re.search(r'\d+',suite.css('.suite-rate .value::text').get().replace('.',''))
                rent='0'
                if regx:
                    rent = regx[0]

                regx = re.search(r'\d+',suite.css('.suite-sqft .value::text').get())
                square_meters='0'
                if regx:
                    square_meters = regx[0]

                regx = re.search(r'\d+',suite.css('.suite-bath .value::text').get())
                bathroom_count='1'
                if regx:
                ...
                regx = re.search(r'\d+',suite.css('.suite-type::text').get())
                room_count='1'
                if regx:
                    room_count = regx[0]
                else:
                ...
                for img in suite.css(".suite-photo::attr(href)").getall():
                    images.append(img)
                floor_plan_images = suite.css(".suite-floorplans a::attr(href)").getall()
