...
class QuotesSpider(scrapy.Spider):
    ...
    country = 'france'
    locale ='fr'
    external_source = "lepointimmobilier_PySpider_france_fr"
    ...
    def get_property_details(self, response):
        ...
        for img in sub_soup.findAll("a",{"class" : "see-photos"}):
            images.append("https://www.lepointimmobilier-agence.fr"+img.find("img").get("src"))
        ...
        square_meters = response.xpath("//li[contains(text(),'Surface :')]/text()").get()
        ...
        if temp_dic.get("nombrepi_ces"):

            item["room_count"] = int(re.findall('\d+',temp_dic["nombrepi_ces"])[0])
        # if temp_dic.get("surface"):
        #     item["square_meters"] = int(re.findall('\d+',temp_dic["surface"])[0])

        item["rent"] = int(re.findall('[\d ]+',temp_dic["prix"])[0].replace('€','').replace(".","").replace(" ",""))
        ...
        if "chargeslocatives" in temp_dic:
