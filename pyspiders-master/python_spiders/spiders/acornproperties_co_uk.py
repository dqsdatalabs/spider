...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self, response):
        ...
        if "let" and "agreed" not in soup.find("h4", class_="property-availability").text.lower():
            ...
            dic["landlord_email"] = "jesmond@acornproperties.co.uk"
            dic["external_source"] = "acornproperties_co_uk_PySpider_unitedkindom_en"
            dic["currency"] = "GBP"
            script_address = response.xpath("//script[contains(.,'propertyAddress =')]/text()").get()
            if script_address:
                address = script_address.split("propertyAddress = '")[1].split("';")[0]
                dic["address"] = address
                dic["city"] = address.split(",")[-2].strip()
                dic["zipcode"] = address.split(",")[-1].strip()
