...
class SportingImmobilierSpider(scrapy.Spider):
    name = 'sporting_immobilier_fr_PySpider_france_fr'
    ...
    def get_details(self, response):
        item = ListingItem()
        item['external_source'] = "sporting_immobilier_fr_PySpider_france_fr"
        ...
        status = response.xpath("//div[contains(@class,'alert-box')]/p[contains(.,'Ce bien est actuellement indisponible')]").get()
        if status:
            return
        yield item
