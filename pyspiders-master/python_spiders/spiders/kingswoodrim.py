...
class spiderQoutes(scrapy.Spider):
    ...
    def get_property_list(self, response, **kwargs):
        ...
        for div in sou.find_all('div',id='page_search'):
            ...
            room = div.find('i',class_='icon-show icon-bed')
            if room:
                rec['room_count'] = int(room.find_next('span').text)
            rec['currency'] = 'GBP'
            ...
            bathroom = div.find('i',class_='icon-show icon-bath')
            if bathroom: 
                rec['bathroom_count'] = int(bathroom.find_next('span').text)
    ...
    def get_property_details(self, response, **kwargs):
        ...
        property_type=response.xpath("//div[@class='single-property-content']/ul//li/text()").get()
        if "garage for rent" in property_type.lower():
            return
        ...
        zipcode = splt_space[-2]+" "+splt_space[-1]
        if "," not in zipcode:
        ...
        item["external_id"] = response.url.split("_")[-1].strip()

        import dateparser
        available_date = response.xpath("//strong[contains(.,'Available')]/text()").get()
        if available_date:
