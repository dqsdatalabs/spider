...
class MySpider(Spider):
    name = 'raywhitemerrylands_com_au'
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://raywhitemerrylands.com.au/properties/residential-for-rent?category=APT%7CUNT&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                    "https://raywhitemerrylands.com.au/properties/residential-for-rent?category=UNT&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "https://raywhitemerrylands.com.au/properties/residential-for-rent?category=HSE%7CTCE%7CTHS&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "house"
            },
            {
                "url" : [
                    "https://raywhitemerrylands.com.au/properties/residential-for-rent?category=STD&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
    ...
    def populate_item(self, response):
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="external_source", input_value="Raywhitemerrylands_Com_PySpider_australia", input_type="VALUE")
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="deposit", input_value="//span[contains(@class,'pdp_price')]/text()[contains(.,'Bond')]", input_type="F_XPATH", get_num=True,split_list={"/":1, " ":0}, replace_list={"$":"", ",":""})
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_name", input_value="Ray White Merrylands", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_phone", input_value="61 (2) 9897 1177", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_email", input_value="merrylands.nsw@raywhite.com", input_type="VALUE")

        if not item_loader.get_collected_values("floor"):
            floor = response.xpath("//li[contains(.,' floor ')]/text()").get()
            if floor: item_loader.add_value("floor", floor.split(' floor ')[0].strip().split(' ')[-1].lower().strip())
