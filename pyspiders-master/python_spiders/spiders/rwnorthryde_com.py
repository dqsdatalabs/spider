...
class MySpider(Spider):
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://raywhitenorthryde.com.au/properties/residential-for-rent?category=APT%7CUNT&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "apartment",
            },
            {
                "url" : [
                    "https://raywhitenorthryde.com.au/properties/residential-for-rent?category=HSE%7CTHS%7CVIL&keywords=&minBaths=0&minBeds=0&minCars=0&rentPrice=&sort=updatedAt+desc&suburbPostCode=",
                ],
                "property_type" : "house"
            },
    ...
    def parse(self, response):
        for item in response.xpath("//a[@class='proplist_item_image_wrap']"):
    ...
    def populate_item(self, response):
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="external_source", input_value="Rwnorthryde_PySpider_australia", input_type="VALUE")
        ...
        dontallow=response.xpath("//span[@class='pdp_price']/text()").get()
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_name", input_value="Ray White North Ryde | Macquarie Park", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_phone", input_value="61 (2) 9889 0266", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_email", input_value="northryde.nsw@raywhite.com", input_type="VALUE")
        
        
        zipcode = response.xpath("//h1[contains(@class,'pdp_address ')]/span/text()").get()
        if zipcode:
            zipcode = zipcode.split(",")[1].strip()
            item_loader.add_value("zipcode", zipcode)

        parking = "".join(response.xpath("//div[@class='pdp_header']//li[@class='car']/span/text()").getall())
        if parking and not item_loader.get_collected_values("parking"):
