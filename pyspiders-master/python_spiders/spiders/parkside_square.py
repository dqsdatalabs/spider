...
class ParksideSquareSpider(scrapy.Spider):
    name = "parkside_square"
    start_urls = ['https://www.parksidesquare.com/floorplans']
    allowed_domains = ["parksidesquare.com"]
    ...
    def parse(self, response, **kwargs):
        ...
        script = soup.find_all('script')[67].text.strip()[15:-1]
        ...
        while index < len(script):
            ...
            if index == -1:
                break
            sqft.append(script[index + 7:index + 10])
            index += 5
        index = 0
        beds = []
        while index < len(script):
        ...
        date = []
        while index < len(script):
            index = script.find('availableDate:', index)
            if index == -1:
                break
            date.append(script[index + 16:index + 26])
            index += 14
        property_urls = ['https://www.parksidesquare.com/availableunits.aspx?myOlePropertyId=1310329&floorPlans=' + x
                         for x in ids]
    ...
    def populate_item(self, response):
        square_meters = int(response.meta['sqft'])
        ...
        address = 'Don Mills and Sheppard'
        ...
        for i in range(count):
            ...
            available_date = response.css("td > span")[i].extract()
            if 'available' in available_date.lower():
                available_date = None
            else:
                available_date = response.meta['date']
            ...
            # # LandLord Details
            item_loader.add_value("landlord_name", 'Parkside Square')  # String
            item_loader.add_value("landlord_phone", '416 900-4070')  # String
