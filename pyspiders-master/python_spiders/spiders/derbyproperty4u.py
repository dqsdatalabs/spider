...
class auditaSpider(scrapy.Spider):
    ...
    def get_property_details(self,response,**kwargs):
        ...
        available_date = response.xpath("//div[@class='property-detail']/p//text()[contains(.,' from the ')]").get()
        if not available_date:
            available_date = response.xpath("//div[@class='property-detail']/p//text()[contains(.,'available from')]").get()
        if available_date:
            available_date = " ".join(available_date.split(" from the ")[-1].split("available from")[-1].replace("around the","").strip().split(" ")[:4])
