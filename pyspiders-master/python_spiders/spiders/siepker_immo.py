...
class SiepkerImmoSpider(scrapy.Spider):
    name = "siepker_immo"
    start_urls = ['https://www.siepker-immobilien.de/immobilienangebot/mieten/']
    allowed_domains = ["siepker-immobilien.de"]
    ...
    def populate_item(self, response):
        ...
        title = response.css('.immo_objectHeadline ::text')[0].extract()
        description = response.css('#immo_detail .col ::text').extract()
        ...
        address = None
        if 'Objektadresse ' in list:
            address = list.split('Objektadresse ')[1].split('Objektart ')[0]
            print(address)
        ...
        # # LandLord Details
        item_loader.add_value("landlord_name", 'Siepker Immobilien') # String
        item_loader.add_value("landlord_phone", '0531-243 33 0') # String
        item_loader.add_value("landlord_email", 'info@siepker-immobilien.de') # String
