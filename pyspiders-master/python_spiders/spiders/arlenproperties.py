...
class auditaSpider(scrapy.Spider):
    ...
    locale ='en'
    external_source = "Arlenproperties_Co_PySpider_united_kingdom"
    ...
    def get_property_details(self,response,**kwargs):
        ...
        item["external_id"] = response.url.split("property-details/")[-1].split("/")[0]

        prop_type = response.xpath("//div[@class='details-address']//span/text()").get()
        if prop_type and "studio" in prop_type.lower(): item["property_type"] = "studio"
        else:
        ...
        available_date = response.xpath("//strong[contains(.,'AVAILABLE FROM')]/text() | //p[contains(.,'AVAILABLE FROM')]/text()").get()
        if available_date:
            date_parsed = dateparser.parse(available_date.split("FROM")[-1].strip(), date_formats=["%d/%m/%Y"], languages=['en'])
            if date_parsed: item["available_date"] = date_parsed.strftime("%Y-%m-%d")
