...
class auditaSpider(scrapy.Spider):
    ...
    def getPropertydtl(self,response,**kwargs):
        ...
        if zipcode=="Terrace, Kensington":
            text=response.xpath("//body/script[contains(.,'address') and contains(.,'MAP')]/text()").get()
            text1=text.replace("\n","").split("address:")[1].split(",")[2].split()[-1].strip().replace('"',"")
            text2=text.replace("\n","").split("address:")[1].split(",")[2].split()[-2].strip()
            zipcode=text1+" "+text2
            item["zipcode"] = zipcode 
