...
class laforet(scrapy.Spider):
    name = 'grechimmo_fr_PySpider_france_fr'
    ...
    def parse(self, response, **kwargs):
        ...
        if num_there(all_page[-1].text.strip()):
            pages = int(all_page[-1].text.rstrip())
        else:
    ...
    def get_property_details(self, response, **kwargs):
        ...
        item["currency"] = "EUR"
        item["external_source"] = "grechimmo_fr_PySpider_france_fr"
        ...
        address2 = response.xpath('/html/head/title/text()').re(r'[Ll]ouer\s(.*?)-')
        if address2:
            item['address'] = address2[0].strip()
        ...
        if soup.find("ul",class_="detail-panel"):
            ...
            if phone:
                item['landlord_phone'] = phone.replace('tel:', '')
