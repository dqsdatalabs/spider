...
class spiderQoutes(scrapy.Spider):
    ...
    def get_page_details(self,response,**kwargs):
        ...
        for ech_prop in all_prop:
            ...
            if "price" in temp_dic: 
                ...
                #response.xpath("//i[@class='price']/following-sibling::text()").get()
    ...
    def get_property_details(self, response, **kwargs):
        ...
        if city_zipcode:
            city = city_zipcode.split(",")[-2]
            zipcode = city_zipcode.split(",")[-1]
            item["city"] = city.strip()
            item["zipcode"] = zipcode.strip()

        rent=response.xpath("//i[@class='price']/following-sibling::text()").get()
        if rent:
        ...
        if square_meters:
            square_meters = square_meters.split("sq")[0].strip().split(" ")[-1].strip()
        ...
        if floor:
            item["floor"] = floor.lower().split("floor")[0].strip()

        available_date = response.xpath("//li[contains(text(),'Available')]/text()").get()
        if available_date:            
            date_parsed = dateparser.parse(available_date.split("Available")[1].replace("from",""), date_formats=["%d/%m/%Y"], languages=['en'])
            if date_parsed: 
