...
class MySpider(Spider):
    ...
    def start_requests(self):
        ...
        for item in start_urls:
            ...
            yield FormRequest(
                "https://www.timothyabrown.co.uk/properties?eapowquicksearch=1&limitstart=0",
                callback=self.parse,
                formdata=formdata,
                dont_filter=True,
                meta={
    ...
    def parse(self, response):
        ...
        if page == 12 or seen:
            ...
            p_url = f"https://www.timothyabrown.co.uk/properties?eapowquicksearch=1&limitstart={page}"
            yield FormRequest(
                p_url,
                callback=self.parse,
                formdata=formdata,
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Timothyabrown_Co_PySpider_united_kingdom")

        external_id = response.xpath("//b[contains(.,'Ref')]/following-sibling::text()").get()
        if external_id:
            item_loader.add_value("external_id", external_id.split(':')[-1].strip())

        address = " ".join(response.xpath("//address//text()").getall()).strip()
        if address:
            item_loader.add_value("address", address.strip())
        
        city_zipcode = response.xpath("//address/text()").get()
        if city_zipcode:
        ...
        if room_count:
            if room_count != "0":
                ...
                room_count = response.xpath("//i[contains(@class,'sofa')]//following-sibling::strong/text()").get()
                if room_count:
                    item_loader.add_value("room_count", room_count.strip())

        bathroom_count = response.xpath("//i[contains(@class,'bath')]//following-sibling::strong/text()").get()
        if bathroom_count:
        ...
        item_loader.add_value("landlord_name", "Timothy A Brown")
        item_loader.add_value("landlord_phone", "01260 271255")
        item_loader.add_value("landlord_email", "contact@timothyabrown.co.uk")
