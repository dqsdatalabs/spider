...
from ..helper import extract_number_only, extract_rent_currency, extract_location_from_address, extract_location_from_coordinates, description_cleaner

class SwRahnImmobilienGbrSpider(scrapy.Spider):
    name = "SW_Rahn_Immobilien_GbR"
    start_urls = [
        'https://www.rahn-immobilien.de/immobilien/page/4/?im_order=datedesc']
    allowed_domains = ["rahn-immobilien.de"]
    ...
    def populate_item(self, response):
        ...
        if 'miete' not in response.css('h2.property-subtitle::text').get().lower():
            return
        ...
        for row in zip(keys, vals):
            ...
            if 'name' in key:
                ...
                rent, currency = extract_rent_currency(val, self.country, SwRahnImmobilienGbrSpider)
                rent = get_price(val)
            elif "warmmiete" in key:
                total_rent, currency = extract_rent_currency(val, self.country, SwRahnImmobilienGbrSpider)
