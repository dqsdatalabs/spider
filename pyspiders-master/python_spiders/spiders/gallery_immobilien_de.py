...
class Gallery_immobilienSpider(scrapy.Spider):
    name = 'gallery_immobilien_de'
    # allowed_domains = ['gallery-immobilien.de']
    start_urls = ['https://smartsite2.myonoffice.de/kunden/weserems/1137/wohnung-miete.xhtml']  # https not http
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response, **kwargs):
        ...
        for rental in rentals:
            if rental.css('.status-reserved') == []:
                ...
                yield Request(url='https://smartsite2.myonoffice.de/kunden/weserems/1137/'+external_link,
                            callback=self.populate_item)
        current_page = response.css('.selected::text').extract_first()
        pages = response.css('a::text').extract()
        if current_page != pages[-1]:
            yield Request(url=f'https://smartsite2.myonoffice.de/kunden/weserems/1137/wohnung-miete.xhtml?f[38133-13]=wohnung&f[38133-11]=miete&f[38133-101]=030&s[38133-447]=1%22%29&p[obj0]={int(current_page)+1}',
                          callback=self.parse)
