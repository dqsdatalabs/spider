...
class MySpider(Spider):
    name = 'century21remicanet_com'
    execution_type='testing'
    country='france'
    locale='fr'
    external_source="Century21remicanet_PySpider_france_fr"
    ...
    def start_requests(self):
        start_urls = [
            {"url": "https://www.century21-remi-canet.com/annonces/location/?types_biens=appartement&xhr=true", "property_type": "apartment"},
	        {"url": "https://www.century21-remi-canet.com/annonces/location/?types_biens=maison&xhr=true", "property_type": "house"},
        ]  # LEVEL 1
    ...
    def parse(self, response):
        ...
        for item in response.xpath("//div[@class='js-the-list-of-properties-list-property']//div[@class='tw-w-full md:tw-w-auto tw-flex-grow']/a/@href").getall():
    ...
    def populate_item(self, response):
        ...
        description = response.xpath("//div[@class='has-formated-text']/text() | //div[@class='has-formated-text tw-inline-block']/text() | //div[@class='has-formated-text md:tw-pl-9']/text()").getall()
        if description:
        ...
        address =response.xpath("//h1//span[contains(@class,'tw-block tw-text-lg')]/text()").get().strip()
        if address:
        ...
        if square_meters:
            square_meters = square_meters.split(":")[1].split("m")[0].replace("\xa0","").strip()
        ...
        images =response.xpath("//div[contains(@class,'c-the-detail-images__item')]/div/img/@data-src").getall()
        if images:
        ...
        if deposit:
           item_loader.add_value("deposit", deposit.split(":")[1].strip("€").replace("\xa0","").replace(" ","").strip())

        

        item_loader.add_value("landlord_phone", "04 68 61 04 72" )        
        item_loader.add_value("landlord_name", "CENTURY 21 Agence Remi")
        item_loader.add_value("landlord_email","dpo@century21france.com")
