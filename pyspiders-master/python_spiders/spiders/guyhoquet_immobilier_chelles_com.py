...
class MySpider(Spider):
    name = 'guyhoquet_immobilier_chelles_com'
    execution_type='testing'
    country='france'
    locale='fr'
    gh_domain = "https://chelles.guy-hoquet.com"
    headers = {
    ...
    def populate_item(self, response):
        ...
        desc="".join(response.xpath("//span[@class='description-more']/text()[contains(.,'Disponible le')]").getall())
        ...
        item_loader.add_value("external_source", "Guyhoquet_Immobilier_Chelles_PySpider_france")
        ...
        import dateparser
        available_date = response.xpath("//span[@class='description-more']/text()[contains(.,'Disponible le')]").get()
        if available_date:
            if not "now" in available_date.lower():
                available_date = available_date.split("Disponible le")[1].strip()
        ...
        if parking:
            ...
            parking="".join(response.xpath("//div[contains(@class,'horaires-item')]//div[contains(.,'Nombre de garages')]//following-sibling::div//text()").getall())
            if parking:
                item_loader.add_value("parking", True)
