...
class CanahahnsComSpider(scrapy.Spider):
    ...
    def populate_item(self, response):
        ...
        for prop in props:
            ...
            if re.search(r"(?=(" + '|'.join(pets_names) + r"))", overview):
                pets_allowed = True

            if re.search(r"(?=(" + '|'.join(balcony_names) + r"))", overview):
                balcony = True

            if re.search(r"(?=(" + '|'.join(terraces) + r"))", overview):
                terrace = True

            if re.search(r"(?=(" + '|'.join(washing_names) + r"))", overview):
                washing_machine = True

            if re.search(r"(?=(" + '|'.join(dishwasher_names) + r"))", overview):
                dishwasher = True

            if re.search(r"(?=(" + '|'.join(park_names) + r"))", overview):
                parking = True

            if re.search(r"(?=(" + '|'.join(elevator_names) + r"))", overview):
                elevator = True

            if re.search(r"(?=(" + '|'.join(pool_names) + r"))", description):
                swimming_pool = True
            ...
            sub_square_feet, sub_bath, sub_rooms, sub_rooms_names, sub_image = self.sub_props(external_link)
    ...
    def sub_props(self, url):
        ...
        sub_image = [re.search(r'(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])', i)[0] if re.search(r'(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])', i) else None for i in  response.css('.info-block:nth-child(6)').extract()]

        return sub_square_feet, sub_bath, sub_rooms, sub_rooms_names, sub_image