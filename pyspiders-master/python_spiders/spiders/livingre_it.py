...
class MySpider(Spider):
    ...
    country='italy'
    locale='it' 
    external_source = "Livingre_PySpider_italy"
    post_urls = ['http://www.livingre.it/web/immobili.asp']  # LEVEL 1

    formdata = {
        "showkind": "",
        "num_page": "1",
        "group_cod_agenzia": "7881",
        "cod_sede": "0",
        "cod_sede_aw": "0",
        "cod_gruppo": "0",
        "pagref": "",
    ...
    def start_requests(self):
        yield FormRequest(self.post_urls[0], callback=self.parse, formdata=self.formdata, headers=self.headers)
    ...
    def populate_item(self, response):
        ...
        reserved = response.xpath("//span[contains(text(),'riservata')]").get()
        if reserved:
            return
        ...
        item_loader.add_value("landlord_name", "Living Re Network Immobiliare")
        item_loader.add_value("landlord_phone", "095336342")
        item_loader.add_value("landlord_email", "canalicchio@livingre.it")

        
        if "Garage" in str(response.body):
            item_loader.add_value("parking",True)

        if "Balcone" in str(response.body):
            item_loader.add_value("balcony",True)


        if "Arredato" in str(response.body):
            item_loader.add_value("furnished",True)
