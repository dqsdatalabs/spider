...
class MySpider(Spider):
    name = 'immobilier_toulouse_rangueil_nestenn_com'
    b_url = "https://immobilier-toulouse-rangueil.nestenn.com"
    execution_type='testing'
    country='france'
    locale='fr'
    external_source = "Immobilier_Toulouse_Rangueil_Nestenn_PySpider_france"
    ...
    def populate_item(self, response):
        ...
        address = response.xpath("//div/a[@property='item']/@href").get()
        if address:
        ...
        if desc:
            ...
            if "de charges" in desc.lower():
                utilities = desc.lower().split("de charges")[0].split("+")[-1].replace("€","").strip().split(" ")[0]
                if utilities:
                    item_loader.add_value("utilities", utilities)
            if "euros de charges" in desc.lower():
                utilities = desc.lower().split("euros de charges")[0].strip().split(" ")[-1].strip().replace("+", " ").split(" ")[-1]
                if utilities:
                    item_loader.add_value("utilities", utilities)
            if "euros charges" in desc.lower():
                utilities = desc.lower().split("euros charges")[0].strip().split(" ")[-1].strip()
                if utilities:
                    item_loader.add_value("utilities", utilities)
        import dateparser
        ...
        item_loader.add_value("landlord_name", "Immobilier Toulouse Rangueil Nestenn")
        item_loader.add_value("landlord_phone", "05 34 31 82 53")
        item_loader.add_value("landlord_email", "toulouse-rangueil@nestenn.com")
