...
class MySpider(Spider):
    name = 'avenirvastgoed_com'
    execution_type = 'testing'
    country = 'netherlands'
    locale = 'nl'
    start_urls = ["https://www.avenirvastgoed.com/nl/realtime-listings/consumer"]
    ...
    def parse(self, response):
        ...
        for item in data:
            if item["isRentals"] and "verhuurd" not in item["status"].lower():
                ...
                term = item["price"] if "price" in item.keys() else ""
                if "p.m" in term.lower(): term = 1
                elif "p.w" in term.lower(): term = 4
                elif "p.d" in term.lower(): term = 30
                else: term = 1
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Avenirvastgoed_PySpider_netherlands")
        ...
        if response.meta.get("rent"): 
            item_loader.add_value("rent", response.meta.get("rent") * response.meta["term"])
        ...
        images = [response.urljoin(x) for x in response.xpath("//div[@class='photo-slider-slides']/img/@src").getall()]
        if images:
        ...
        item_loader.add_value("landlord_name", "Avenir Vastgoed")
        item_loader.add_value("landlord_phone", "070-820 98 92")
        item_loader.add_value("landlord_email", "info@avenirvastgoed.com")

        if not item_loader.get_collected_values("description"):
            description = " ".join(response.xpath("//p[@class='object-description']//text()").getall()).strip()   
            if description: item_loader.add_value("description", description.replace('\xa0', ''))
        
        if not item_loader.get_collected_values("deposit"):
            deposit = response.xpath("//text()[contains(.,'Deposit ')]").get()
            if deposit: item_loader.add_value("deposit", "".join(filter(str.isnumeric, deposit)))

        if response.xpath("//dt[contains(.,'Parkeren')]/following-sibling::dd[1]/text()[contains(.,'Parkeerplaats') or contains(.,'Garage')]").get():
