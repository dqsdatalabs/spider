...
class Wuppertal_homecompanyDeSpider(scrapy.Spider):
    name = 'wuppertal_homecompany_de'
    allowed_domains = ['wuppertal.homecompany.de']
    start_urls = ['https://wuppertal.homecompany.de/en/suchergebnis/noreset/1']  # https not http
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response, **kwargs):
        ...
        for rental in rentals:
            yield Request(url='https://wuppertal.homecompany.de'+rental,
                          callback=self.populate_item)
        if response.css('.nextpage'):
            next_page = response.css('.nextpage::attr(href)').extract_first()
            yield Request(url='https://wuppertal.homecompany.de/'+next_page,
                          callback=self.parse)
    ...
    def populate_item(self, response):
        ...
        if len(description) == 0:
            description = remove_white_spaces((((' '.join(
                response.css("table.detailsTable th:contains('object information') + td ::text").getall()).replace('\n','')).replace('\t', '')).replace('\r', '')))
        ...
        images = ['https://wuppertal.homecompany.de/'+i for i in images]
        ...
        landlord_name = "wuppertal homecompany"
        landlord_phone = "+ 49 (0) 234 - 19445"
        landlord_email = "wuppertal@homecompany.de"
