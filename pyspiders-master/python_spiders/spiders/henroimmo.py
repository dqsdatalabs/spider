...
class HenroimmoSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        next_page = response.xpath("//li[@class='page-item']//a[.='»']/@href").extract_first()
        if next_page:
            yield scrapy.Request(
    ...
    def get_details(self, response):
        ...
        external_id = ''.join(response.css("li:contains('REF')::text").extract())
        if external_id:
            item['external_id'] = external_id.split(" ")[-1]
        bathroom_count = response.xpath("//ul[@class='padding-0']/li[contains(.,'Salle de douche') or contains(.,'Salle de bain')]/text()").get()
        if bathroom_count:
            item['bathroom_count'] = int(bathroom_count.split(":")[1].strip())
