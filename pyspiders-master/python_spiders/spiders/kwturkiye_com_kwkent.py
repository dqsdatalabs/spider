...
class MySpider(Spider):
    ...
    def populate_item(self, response):
        ...
        if"sezonluk" in (data['result']['listing']['listingHeader']).lower():return
        item_loader.add_value("external_source", "Kwturkiyekent_PySpider_"+ self.country + "_" + self.locale)

        if data["result"]["listing"]["bathRooms"]: item_loader.add_value("bathroom_count", int(data["result"]["listing"]["bathRooms"]))
        ...
        if rent:
            item_loader.add_value("rent", str(int(rent)))
        ...
        params = urllib.parse.quote(ext_url.split('kwturkiye.com/')[1])
        item_loader.add_value("external_link", "https://www.kwturkiye.com/" + params) 
