...
class MySpider(Spider):
    name = 'immo_beratto_com'
    execution_type='testing'
    country='france'
    locale='fr'
    # DYNAMIC VALUES
    gh_domain = "https://villers-cotterets.guy-hoquet.com"
    source_name = "Immo_Beratto_PySpider_france"
    landlord_n = "GUY HOQUET VILLERS COTTERETS"
    landlord_p = "0364183800"
    # --------------
    headers = {
        ...
        'x-csrf-token': "zeKJ2qdT7nlUj3doFGECDLgk7p2tyjWGC1iIN0zX",
    ...
    def populate_item(self, response):
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="room_count", input_value="//div[@id='intro']//i[contains(@class,'ico king_bed')]/following-sibling::div/text()", input_type="F_XPATH", get_num=True)
        ...
        city = response.xpath("//div[@class='add']/text()").get()
        if city: item_loader.add_value("city", " ".join(city.strip().split(" ")[:3]).strip())
