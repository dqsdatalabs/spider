...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        item["description"] = description.replace("Immosquare","").replace("immosquare","").replace("immosQuare","").replace("IMMOSQUARE","")

        externalid=response.url
        if externalid:
            if "GES" in externalid:
                item["external_id"]=(externalid.split("-")[-2]+"-"+externalid.split("-")[-1]).split(".")[0]
            else:
                item["external_id"]=externalid.split("-")[-1].split(".")[0]
        ...
        item["landlord_phone"] = "04 56 40 60 10"
