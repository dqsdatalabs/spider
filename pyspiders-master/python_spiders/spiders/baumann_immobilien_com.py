...
from ..helper import extract_number_only, extract_location_from_address, \
    extract_location_from_coordinates
from ..loaders import ListingLoader


class Baumann_immobilienComSpider(scrapy.Spider):
    name = 'baumann_immobilien_com'
    # allowed_domains = ['baumann-immobilien.com']
    start_urls = ['https://portal.immobilienscout24.de/ergebnisliste/30738745',  # apartments
                  'https://portal.immobilienscout24.de/ergebnisliste/52347185']  # houses
    country = 'germany'
    locale = 'de'
    ...
    def parse(self, response, **kwargs):
        ...
        if response.css('.is24portale-next-rel'):
            next_page = response.css('.is24portale-next-rel--link::attr(href)').extract_first()
            if next_page:
                yield Request(url='https://portal.immobilienscout24.de'+next_page,
                            callback=self.parse)
    ...
    def populate_item(self, response):
        ...
        description = ((((' '.join(response.css('.expose--text~ .expose--text+ .expose--text h4+ p::text').extract()).replace('\n','')).replace('\t','')).replace('\r','')))
        ...
        if 'Bezugsfrei ab:' in item_list.keys():
            ...
            if available_date == 'nach Absprache':
                available_date = None
        ...
        elevator = None
        if 'Personenaufzug:' in item_list.keys():
            elevator = True
        if any(word in description.lower() for word in self.keywords['elevator']):
            elevator = True
        ...
        balcony = None
        terrace = None
        if 'Balkon/ Terrasse:' in item_list.keys():
            balcony = True
            terrace = True
        ...
        # # LandLord Details
        item_loader.add_value("landlord_name", 'Mr. Bernd Baumann') # String
        item_loader.add_value("landlord_phone", '0171 7171056') # String
        item_loader.add_value("landlord_email", 'info@baumann-immobilien.com') # String
