...
class auditaSpider(scrapy.Spider):
    ...
    def parse2(self, response, **kwargs):
        ...
        if available_date:            
            date_parsed = dateparser.parse(available_date.split("AVAILABLE")[-1].replace("FROM","").replace(" MID",""), date_formats=["%d/%m/%Y"], languages=['en'])
            if date_parsed:
        ...
        parking = "".join(response.xpath("//li[@class='lIhZ24u1NHMa5Y6gDH90A']/text()[contains(.,'PARKING')]").extract())
        if parking:
            item["parking"] = True
