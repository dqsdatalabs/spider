...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self, response):
        ...
        title = sub_soup.find("div", {"class" : "row"}).find("h2").text
        item["title"] = title
        address = item["title"].replace(item["title"].split("-")[-1],"").strip()
        ...
        item["landlord_name"] = "LG IMMOBILIER"
        ...
        if response.url == "http://www.lg-immobilier.fr/bien/amiens-metropole-longueau-type-1-avec-parking-6":
            return
