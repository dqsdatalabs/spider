...
from python_spiders.helper import extract_rent_currency, format_date, extract_number_only, remove_white_spaces
from datetime import date
import lxml 
import js2xml
from scrapy import Selector

class SandersonJames(scrapy.Spider):

    name = "sanderson_james_co_uk"
    ...
    def parse(self, response, **kwargs):
        ...
        for property_item in listings:
            ...
            if property_item.xpath('.//strong[contains(text(),"Let Agreed")]').extract_first() or \
                'per month' not in property_item.xpath('.//div[@class="listing-property-price"]/text()').extract_first().lower():
                print('LET AGREED')
                continue
    ...
    def get_property_details(self, response):
        ...
        if title:
            item_loader.add_value('room_count',extract_number_only(title.split('Bedrooms :')[-1]))
