...
class ImmobilienKontorSpider(scrapy.Spider):
    name = "Immobilien_Kontor"
    start_urls = [
        'https://immobilien-kontor.org/immobilien-vermarktungsart/miete']
    ...
    def parse(self, response, **kwargs):
        listings =  response.css("div.property")
        for listing in listings:
            url = listing.css("h3> a::attr(href)").get()
            status = listing.css('div.property-status::text').get()
            if status:
                if 'vermietet' not in status and 'reserviert' not in status:
                    yield scrapy.Request(url, callback=self.populate_item, meta={"title": listing.css('h3.property-title a::text').get()})
    ...
    def populate_item(self, response):
        heating_cost =  washing_machine = address = property_type = pets_allowed = balcony = terrace = elevator = external_id = floor = parking = energy_label = None
        ...
        for row in zip(keys, vals):
            ...
            if 'name' in key:
                ...
                address = val
            elif "etage" in key and 'haus' not in key:
                floor = val.strip()[0]
                ...
                rent, currency = extract_rent_currency(val, self.country, ImmobilienKontorSpider)
                rent = get_price(val)
            elif "warmmiete" in key:
                total_rent, currency = extract_rent_currency(
                    val, self.country, ImmobilienKontorSpider)
                total_rent = get_price(val)
            elif "nebenkosten" in key or 'betriebskosten' in key:
        ...
        if landlord_phone is None:
            landlord_phone = "02932-97200"
        if rent is None:
            rent = total_rent
        if total_rent:
            heating_cost = total_rent - rent
