...
class harveyResidental(scrapy.Spider):
    ...
    def get_pageDetails(self, response, **kwargs):
        ...
        for i in link_:
            link_in = i["href"]
            # link_in = 'http://harveyresidential.com/listings/stamford-works-gillett-street-dalston-n16-47500-p-a/'
    ...
    def get_property_details(self, response, **kwargs):
        ...
        soup = BeautifulSoup(response.body,"html.parser")      
        # print (response.url)
        rented = response.xpath("//div/b[.='Price:']/following-sibling::text()[1][.=' RENTED']").extract_first()
        if rented:
            return
        ...
        if "price" in tmp_dic and num_there(tmp_dic["price"]):
            ...
            elif "pm"  in tmp_dic["price"]:
                item["rent"] = getPrice(tmp_dic["price"])
            elif "p.a"  in tmp_dic["price"]:
                item["rent"] = int(getPrice(tmp_dic["price"])/12)
            else:
                item["rent"] = getPrice(tmp_dic["price"])    
                         
        if not item.get('rent'):
            rent = response.xpath("//h1/text()[contains(.,'£')]").extract_first()
            if rent:                                           
        ...
        if "address" in tmp_dic:
            ...
            item['address'] = response.xpath("//div[@class='entry-content']/p/text()").extract_first().split('–')[0]
        if "city" in tmp_dic and tmp_dic["city"].strip():
            item["city"] = tmp_dic["city"].strip()
        else:
            if len(item['address'].split(',')) > 1:
                item['city'] = item['address'].split(',')[-2]
        if "postcode" in tmp_dic:
                item["zipcode"] = tmp_dic["postcode"].strip()
        else:
            if not item['address'].split(',')[-1].replace(" ","").isalpha():
                item['zipcode'] = item['address'].split(',')[-1]
        if "squarefeet" in tmp_dic and num_there(tmp_dic["squarefeet"]):
            item["square_meters"] = int(getSqureMtr(tmp_dic["squarefeet"].strip())*0.092)
        ...
        if "basement" in tmp_dic and tmp_dic["basement"].strip():
            item["floor"] = tmp_dic["basement"].strip()
            item['floor'] = item['floor'].replace("No.","")
