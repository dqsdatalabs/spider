...
class EukiaverwaltungsgmbhSpider(scrapy.Spider):
    name = "EUKIAVerwaltungsGmbH"
    start_urls = ['https://www.eukia-hausverwaltung.de/mietangebote.htm']
    allowed_domains = ["eukia-hausverwaltung.de"]
    ...
    def parse(self, response, **kwargs):
        for listing in  response.css("div.item"):
            if listing.css("div.title::text").get() is not None:
                address =  response.css("div.item")[0].css("div.sufar strong::text").get()
                if address is not None:
                    yield scrapy.Request('https://www.eukia-hausverwaltung.de' + listing.css("a::attr(href)").get(), callback=self.populate_item, meta={'address': address.replace('|', ',')})
    ...
    def populate_item(self, response):
        ...
        if not 'apartment' in title.lower() and not 'wohnung' in title.lower():
            return
        property_type = 'apartment'
        external_id = response.url.split("_")[-1].split('.')[0]
        address = response.meta['address']
        
        try:
            temp_description = []
            temp_description.extend([x.strip() for x in response.css("div.data p::text").getall()])
            temp_description.extend([x.strip() for x in response.css("div.description p::text").getall()])
            description = '\n'.join([x.strip() for x in temp_description if '@' not in x and ' unter ' not in x and ' Tel' not in x and x.strip() != '' and 'eukia' not in x.lower()]).strip()
        except:
            description = ''
        ...
        for row in zip(keys, vals):
            ...
            if "zimmer" in key:
                ...
                rent, currency = extract_rent_currency(val, self.country, EukiaverwaltungsgmbhSpider)
                rent = get_price(val)
            ...
            if 'objektnummer' in key:
                ...
                total_rent, currency = extract_rent_currency(val, self.country, EukiaverwaltungsgmbhSpider)
        ...
        images = ['https://www.eukia-hausverwaltung.de'+x for x in response.css("a[rel='slideshow1']::attr(href)").getall()]
        floor_plan_images = None

        landlord_phone = '+49941920280'
        landlord_email = 'info@eukia-hausverwaltung.de'
        landlord_name = 'EUKIA Vermietungs- und Verwaltungs GmbH'
