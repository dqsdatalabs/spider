...
class MySpider(Spider):
    name = 'lovelondonproperty_co_uk'
    execution_type='testing'
    country='united_kingdom'
    locale='en'
    start_urls = ['https://www.lovelondonproperty.co.uk/properties-to-let']  # LEVEL 1
    ...
    def parse(self, response):
        ...
        if page == 12 or seen:
            url = f"https://www.lovelondonproperty.co.uk/properties-to-let?start={page}"
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Lovelondonproperty_Co_PySpider_united_kingdom")
        ...
        zipcode = response.xpath("//div[@class='eapow-sidecol eapow-mainaddress']/address/text()[last()]").get()
        if zipcode:
        ...
        if name:
            ...
            item_loader.add_value("landlord_name", "Love London Property")
        
        phone = response.xpath("//b[contains(.,'T:')]/following-sibling::text()").get()
        if phone:
            item_loader.add_value("landlord_phone", phone)
        else:
            item_loader.add_value("landlord_phone", "0203 417 4080")
            
        email = response.xpath("//a[contains(@href,'mailto')]/text()").get()
        if email:
            item_loader.add_value("landlord_email", email)
        else:
            item_loader.add_value("landlord_email", "ask@lovelondonproperty.co.uk")
