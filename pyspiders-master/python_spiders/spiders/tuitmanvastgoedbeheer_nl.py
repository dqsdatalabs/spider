...
class MySpider(Spider):
    ...
    external_source = "Tuitmanvastgoed_PySpider_netherlands"
    def start_requests(self):
        
        formdata = {
            "action": "all_locations",
            "api": "31db0486a76a2afca54e18a3a214bc4b",
    ...
    def parse(self, response):
        ...
        for item in data:
            if item.isdigit():
                status = data[item]["front_status"]
                if status and "verhuurd" in status.lower():
                    continue
                follow_url = f"https://www.tuitmanvastgoedbeheer.nl/woning?{data[item]['city']}/{data[item]['street']}/{data[item]['house_id']}".replace(" ", "-")
    ...
    def get_property_info(self, response):
        ...
        item_loader.add_value("external_source", "Tuitmanvastgoed_PySpider_netherlands")
        ...
        item_loader.add_value("landlord_name", "Tuitman Vastgoedbeheer")
        item_loader.add_value("landlord_phone", "050 7518550")
        item_loader.add_value("landlord_email", "info@tuitmanvastgoed.nl")
