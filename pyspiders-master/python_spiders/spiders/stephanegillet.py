    ...
    def parse(self, response):
        ...
        if page ==2 or seen:
            if response.meta.get('property_type') == "apartment":
                url = f"https://www.stephanegillet.be/destination/appartementlocation/page/{page}/"
                yield scrapy.Request(url, callback=self.parse, meta={"page":page+1, "property_type":response.meta.get('property_type')})
            elif response.meta.get('property_type') == "house":
                url = f"https://www.stephanegillet.be/destination/maisonslocation/page/{page}/"
                yield scrapy.Request(url, callback=self.parse, meta={"page":page+1, "property_type":response.meta.get('property_type')})
