...
class auditaSpider(scrapy.Spider):
    ...
    def get_property_details(self,response,**kwargs):
        ...
        if title:
            if "," in title:
                if title.split(",")[-1].strip().isalpha():
                    item["city"] = title.split(",")[-1].strip()
                elif title.split(",")[-2].strip():
                    item["city"] = title.split(",")[-2].strip()
        ...
        if "," in temp_zip:
            if num_there(temp_zip.split(",")[-1]):
                zipcode = temp_zip.split(",")[-1].strip()
                if "*" in zipcode:
                    zipcode = zipcode.split("*")[0].strip()
                
                item["zipcode"] = zipcode

        item["address"] = response.xpath("//h1/text()").get()
            ...
            for ech_wrapper in soup.find("div", class_="single-property-wrapper").find_all("div", class_="meta-inner-wrapper"):
                ...
                if "status" in ech_wrapper.find("span", class_="meta-item-label").text.lower():
                    if "rent" in ech_wrapper.find("span", class_="meta-item-value").text.lower():
                        ...
                        if extract_data and "lat" in extract_data[0].split(",")[0]:
                            item["latitude"] = extract_data[0].split(",")[0].split(":")[-1].replace('"','')
                        if extract_data and "lang" in extract_data[0].split(",")[1]:
