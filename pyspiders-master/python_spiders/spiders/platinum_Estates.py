...
class spiderQoutes(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        description = "".join(response.xpath("//div[@class='description-property']/p//text()").extract())
        if description:
            item['description'] = re.sub("\s{2,}", " ", description)

        else:
            description = "".join(response.xpath("//div[@class='layer-info']//text()").extract())
            if description:
                item['description'] = re.sub("\s{2,}", " ", description)
        desccheck= "".join(response.xpath("//div[@class='description-property']/p//text()").extract())
        if desccheck and "workshop" in desccheck.lower():
            return 
