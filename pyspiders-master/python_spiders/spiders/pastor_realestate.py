...
class QuotesSpider(scrapy.Spider):
    ...
    def get_property_details(self, response, **kwargs):
        ...
        title = soup.find("div", class_="col-md-12")
        if title:
            title = title.find("h1").text.strip()
        item["title"] = title

        address = soup.find("div", class_="col-md-12")
        if address:
            address = address.find("h2").text.strip()
            item["address"] = address
            item["city"] = address.split(",")[-2]
            item["zipcode"] = address.split(",")[-1] 


        if soup.find("div", class_="col-md-12"):
            rent = getPrice(soup.find("div", class_="col-md-12").find("p", class_="item-price").text)
            # rent = (rent/7)*30
            item["rent"] = rent * 4

        details = soup.find("div", class_="tab-content details")
        if details:
            details = details.find("div", class_="property-overview-content")
            ...
            if list_text and int(list_text[0]) > 100:
        ...
        image_list = []
        imgs = soup.find("div",class_="slider slider-modal-gallery js-parent-slider")
        if imgs:
            imgs = imgs.find_all("div", class_="slide")
            for im in imgs:
