...
class MySpider(Spider):
    name = 'century21_ab_dinan'
    external_source = "Century21AbDinan_PySpider_france"

    def start_requests(self):
        start_urls = [
            {
                "url": [
                    "https://www.century21-ab-dinan.com/annonces/location-appartement/",
                ],
                "property_type": "apartment"
            },
	        {
                "url": [
                    "https://www.century21-ab-dinan.com/annonces/location-maison/"
                ],
                "property_type": "house"
            },
        ]  # LEVEL 1
    ...
    def parse(self, response):
        ...
        for item in response.xpath("//a[contains(.,'détail')]/@href").extract():
    ...
    def populate_item(self, response):
        ...
        terrace = response.xpath("//div[contains(.,'Terrasse')]/text()[normalize-space()]").getall()
        if terrace:
            item_loader.add_value("terrace", True)
        parking = response.xpath("//div[contains(.,'parking')]/text()[normalize-space()]").getall()
        if parking:
        ...
        energy_label = response.xpath("//span[@class='tw-block tw-m-auto']/text()").get()
        if energy_label:
            energy_label = find_energy_label(energy_label)
        ...
        item_loader.add_value("landlord_name", "CENTURY 21 Agence Gambetta")
