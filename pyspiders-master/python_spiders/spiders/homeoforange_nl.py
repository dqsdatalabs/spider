...
class MySpider(Spider):
    name = 'homeoforange_nl'
    execution_type = 'testing'
    country = 'netherlands'
    locale = 'nl'
    start_urls = ["https://www.homeoforange.nl/nl/realtime-listings/consumer"]
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Homeoforange_PySpider_netherlands")
        ...
        images = [response.urljoin(x) for x in response.xpath("//div[@class='responsive-slider-slide']/img/@src").getall()]
        if images:
        ...
        if utilities:
            ...
            utilities = response.xpath("//h2[contains(.,'Omschrijving')]/following-sibling::p//text()[contains(.,'Servicekosten') and contains(.,'€')]").get()
            if utilities:
                item_loader.add_value("utilities", utilities.split('€')[-1].split(",")[0].strip())

        deposit = response.xpath("//dt[contains(.,'Borg')]/following-sibling::dd[1]/text()").get()
        if deposit:
        ...
        item_loader.add_value("landlord_name", "Home of Orange")
        item_loader.add_value("landlord_phone", "020 - 24 431 20")
        item_loader.add_value("landlord_email", "hello@homeoforange.nl")
