...
class MySpider(Spider):
    ...
    country='australia'
    locale='en' 
    external_source='Rwssyd_PySpider_australia'
    custom_settings = {
    ...
    def start_requests(self):
        ...
        url = "https://www.rwaw.com.au/wp-admin/admin-ajax.php"
    ...
    def parse(self, response):
        ...
        for item in data.split('img-container'):
            item=item.split('title')[0].replace(" href=","").replace("\\","").replace('"',"")
    ...
    def populate_item(self, response):
        ...
        adres=response.xpath("//h1[@class='property-address text-center color-primary']/text()").get()
        if adres:
        ...
        if rent and not 'DEPOSIT TAKEN':
            item_loader.add_value("rent",int(rent.split("$")[-1].split("pw")[0].strip())*4)
        ...
        property_type=response.xpath("//h3[@class='sub-title color-primary']/text()").get()
        ...
        name=response.xpath("//div[@class='agent-detail w-30']/a//p/text()").get()
        if name:
