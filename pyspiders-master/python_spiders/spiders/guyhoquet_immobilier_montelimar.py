...
class MySpider(Spider):
    name = 'guyhoquet_immobilier_montelimar'
    execution_type='testing'
    country='france'
    locale='fr'
    external_source = "Guyhoquet_Immobilier_Montelimar_PySpider_france"
    gh_domain = "https://montelimar.guy-hoquet.com"
    headers = {
    ...
    def get_token(self, response):
        ...
        start_urls = [
            {"url": f"{self.gh_domain}/biens/result?1&templates%5B%5D=properties&p=1&t=&filters%5B10%5D%5B%5D=2&filters%5B30%5D%5B%5D=appartement&with_markers=false&with_map=false&&templates%5B%5D=properties&p=1&t=&filters%5B10%5D%5B%5D=2&filters%5B30%5D%5B0%5D%5B%5D=appartement&with_markers=false&with_map=false&_=1632823897681", 
             "property_type": "apartment"},
	        {"url": f"{self.gh_domain}/biens/result?1&templates%5B%5D=properties&p=1&t=&filters%5B10%5D%5B%5D=2&filters%5B30%5D%5B%5D=appartement&with_markers=false&with_map=false&&templates%5B%5D=properties&p=1&t=&filters%5B10%5D%5B%5D=2&filters%5B30%5D%5B0%5D%5B%5D=maison&with_markers=false&with_map=false&_=1632823897683", 
            "property_type": "house"}
        ] 
    ...
    def parse(self, response):
        ...
        if int(data["total"]) > len(data_url):
            if page == 2 or seen:
    ...
    def populate_item(self, response):
        ...
        desc="".join(response.xpath("//div[h2[.='Descriptif ']]//span[@class='description-more']/text()[normalize-space()]").getall())
