...
class MySpider(Spider):
    ...
    def start_requests(self):
        start_urls = [
            {
                "url" : [
                    "https://www.maresidences.nl/woningaanbod/huur/type-appartement",
                ],
    ...
    def parse(self, response):
        for item in response.xpath("//span[contains(@class,'object_status') and not(contains(@class,'rented'))]/following-sibling::a/@href").getall():
    ...
    def populate_item(self, response):
        ...
        item_loader.add_value("external_source", "Maresidences_PySpider_netherlands")
        item_loader.add_xpath("title", "//div[@class='object_title']/h1/text()")
 
        address =response.xpath("substring-after(//div[@class='object_title']/h1/text(),':')").extract_first()
        if address:
        ...
        rent =response.xpath("//span[@class='object_price']/text()").extract_first()
        if rent:     
        ...
        item_loader.add_value("landlord_name", "MA Residences")
        item_loader.add_value("landlord_phone", "020-3418648")
        item_loader.add_value("landlord_email", "info@maresidences.nl")
