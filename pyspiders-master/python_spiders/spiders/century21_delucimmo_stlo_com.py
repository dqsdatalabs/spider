...
class MySpider(Spider):
    name = 'century21_delucimmo_stlo_com'
    execution_type ='testing'
    country ='france'
    locale ='fr'
    external_source = "Century21_Delucimmo_St_Lo_PySpider_france"
    custom_settings = {
    ...
    def start_requests(self):
        start_urls = [
            {"url": "https://www.century21-delucimmo-st-lo.com/annonces/location-appartement/", "property_type": "apartment"},
	        {"url": "https://www.century21-delucimmo-st-lo.com/annonces/location-maison/", "property_type": "house"},
        ]  # LEVEL 1
    ...
    def populate_item(self, response):
        ...
        price = response.xpath("//p[@class='c-the-property-abstract__price c-text-theme-heading-1 is-constant tw-tracking-c21-theme-40 tw-my-2 tablet-landscape:tw-my-0']//text()").get()
        if price:
            item_loader.add_value("rent", price.split("€")[0].replace(" ","").strip())
        ...
        utilities = response.xpath("//ul[@class='c-list md:tw-pl-14']//p[contains(.,'pour charges')]//text()").get()
        if utilities:
        ...
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_name", input_value="CENTURY 21 Delucimmo", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_phone", input_value="02 33 05 91 90", input_type="VALUE")
        ItemClear(response=response, item_loader=item_loader, item_name="landlord_email", input_value="delucimmo@century21.fr", input_type="VALUE")
