...
class QuotesSpider(scrapy.Spider):
    ...
    locale = 'en'
    external_source='Adamsestates_PySpider_united_kingdom'
    ...
    def get_property_details(self, response):
        ...
        city=response.xpath("//h1[@id='banner-title']/text()").get()
        if city:
        ...
        deposit = "".join(response.xpath("//ul[@class='property-features col-sm-6']/li[contains(.,'DEPOSIT')]/text()").extract())
        if deposit:
            dep = deposit.strip().split(" ")[-1].replace("£","").replace(".00.","").strip()
            item["deposit"] = dep
        images = []
        ...
        available_date=response.xpath("substring-before(substring-after(//h2/text()[contains(.,'Available')],'Available '),' -')").get()
        if available_date:
        ...
        if "furnish" in description.lower():
            ...
            furnished = "".join(response.xpath("//h2/text()[contains(.,'Furnished')]").extract())
            if furnished:
                item["furnished"]=True
        ...
        item["landlord_name"] = 'ADAMS ESTATES'
        item["landlord_phone"] = '01189 351616'
        item["landlord_email"] = 'leads@adamsestates.net'
